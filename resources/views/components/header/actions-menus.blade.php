...
{{-- Albums selection --}}
<x-header.button x-cloak x-show="selectedAlbums.length === 1" x-on:click="renameAlbums" icon="pencil" />
...
<x-header.button x-cloak x-show="selectedPhotos.length > 0" x-on:click='' icon='tag'/>{{-- tag --}}
<x-header.button x-cloak x-show="selectedPhotos.length === 1" x-on:click='' icon='folder-cover'/>{{-- setAsCover --}}
<x-header.button x-cloak x-show="selectedPhotos.length > 0" x-on:click='' icon='pencil'/>{{-- rename --}}
<x-header.button x-cloak x-show="selectedPhotos.length > 0" x-on:click='' icon='layers'/>{{-- copyTo --}}
<x-header.button x-cloak x-show="selectedPhotos.length > 0" x-on:click='' icon='transfer'/>{{-- move --}}
<x-header.button x-cloak x-show="selectedPhotos.length > 0" x-on:click='' icon='trash'/>{{-- delete --}}
<x-header.button x-cloak x-show="selectedPhotos.length > 0" x-on:click='' icon='cloud-download'/>{{-- download --}}
@endcan
</div>