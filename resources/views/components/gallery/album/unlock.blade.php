<div class="basicModalContainer transition-opacity duration-1000 ease-in animate-fadeIn
    ...
    <div class="basicModal transition-opacity ease-in duration-1000
        ...
        <div class="flex w-full box-border">
            <x-forms.buttons.cancel class="border-t border-t-dark-800 rounded-bl-md w-full" x-on:click="$wire.back()">{{ __('lychee.CANCEL') }}</x-forms.buttons.cancel>
            <x-forms.buttons.action class="border-t border-t-dark-800 rounded-br-md w-full" wire:click="unlock">{{ __('lychee.ENTER') }}</x-forms.buttons.action>
        </div>
    </div>
</div>
