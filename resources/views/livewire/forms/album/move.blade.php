<div class="text-neutral-200 text-sm p-9 text-center w-full max-w-3xl h-24" x-data="{ isOpen: true }" x-on:click.away="isOpen = false">
	@if ($title !== null && $title !== '')
	<p class="mb-4 text-center">{{ sprintf(__('lychee.ALBUM_MOVE'), $titleMoved, $title) }}</p>
	<x-forms.buttons.action class="rounded-md w-full" wire:click='move'>{{ __('lychee.MOVE_ALBUM') }}</x-forms.buttons.danger>
	@else
	<div class="w-full">
		<div class="w-full">
					...
					<ul class=" max-h-[50vh] overflow-y-auto">
						@foreach ($this->albumList as $result)
							<li class="border-b border-neutral-700 cursor-pointer transition-all ease-in-out duration-300
								...
								wire:click="check('{{ $result['id'] }}', '{{ $result['original'] }}')">
