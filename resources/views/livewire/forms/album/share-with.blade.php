    ...
    <div class="mt-4" x-data="{ isSearchUserOpen: false }" x-on:click.away="isSearchUserOpen = false">
        <form>
            @if ($username !== null)
                ...
                <span class="h-4 w-56 inline-block relative mt-2.5 cursor-pointer">{{ $username }} <a
                        class="block absolute right-0 top-0" wire:click="clearUsername()">
                        <x-icons.iconic class=" fill-red-800 w-4 h-4 mb-2" icon="x" /></a>
                </span>
                <span class="w-56 inline-block text-center">
                    <x-forms.tickbox title="User can access picture in full size"
                        wire:model.live='grants_full_photo_access' />
                ...
                <x-forms.buttons.create class="rounded w-20"
                    wire:click='add'>{{ __('lychee.ADD') }}</x-forms.buttons.action>
                @else
				...
				if (event.key === '/') {
					...
					$refs.search.focus();
                            ...
                            @focus="isSearchUserOpen = true" @keydown="isSearchUserOpen = true"
                            @keydown.escape.window="isSearchUserOpen = false"
                            @keydown.shift.tab="isSearchUserOpen = false">
                        <span class="absolute block top-0">
                    ...
                    <div class="z-50 absolute bg-neutral-800 text-xs rounded w-56 mt-4"
                        x-show.transition.opacity="isSearchUserOpen">
