            ...
            <div id="imageview" class="absolute top-0 left-0 w-full h-full bg-black " x-data="{
                ...
                @if ($photo->isVideo()) {{-- This is a video file: put html5 player --}}
					...
					{{ $visibleControls === true ? '' : 'full' }}'
                        autobuffer {{ $autoplay ? 'autoplay' : '' }} data-tabindex='{{ Helpers::data_index() }}'>
                        <source src='{{ URL::asset($photo->size_variants->getOriginal()->url) }}'>Your browser does not
