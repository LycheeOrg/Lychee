<header id="lychee_toolbar_container" @class([
	...
	@if ($gallery_mode === App\Enum\Livewire\GalleryMode::ALBUMS) <!-- ALBUMS -->
		...
		<div id="lychee_toolbar_public" class="toolbar visible">
			<a class="button" wire:click="openLoginModal" id="button_settings"><x-icons.iconic icon="account-login" />
				{{-- <svg class="iconic"><use xlink:href="#account-login" /></svg> --}}
			</a>
		...
		<div id="lychee_toolbar_albums" class="toolbar visible">
			<a class="button" wire:click="$emitTo('pages.gallery','openLeftMenu')" id="button_settings"><svg class="iconic"><use xlink:href="#cog" /></svg></a>
			<a class="header__title">{{ $title }}</a>
			<div class="header__search__field">
				<input class="header__search" type="text" name="search" placeholder="Search …">
				<a class="header__clear">&times;</a>
	...
	@elseif ($gallery_mode === App\Enum\Livewire\GalleryMode::ALBUM) <!-- ALBUM -->
		<div id="lychee_toolbar_album" class="toolbar visible">
			<a class="button" id="button_back_home" title="Close Album" wire:click="$emitTo('pages.gallery','back')"><svg class="iconic"><use xlink:href="#chevron-left" /></svg></a>
			<a class="header__title">{{ $title }}</a>
	...
	@elseif ($gallery_mode === App\Enum\Livewire\GalleryMode::PHOTO) <!-- PHOTO -->
		<div id="lychee_toolbar_photo" class="toolbar visible">
			<a class="button" id="button_back" wire:click="$emitTo('pages.gallery','back')"><svg class="iconic"><use xlink:href="#chevron-left" /></svg></a>
	...
	@elseif ($page_mode === App\Enum\Livewire\PageMode::SETTINGS) <!-- SETTINGS -->
		<div id="lychee_toolbar_config" class="toolbar visible">
			<a class="button" id="button_close_config"><svg class="iconic"><use xlink:href="#chevron-left" /></svg></a>
			<a class="header__title">{{ $title }}</a>
		</div>
	@endif
</header>