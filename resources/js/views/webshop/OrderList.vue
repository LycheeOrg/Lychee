<template>
	...
	<Panel :pt:header:class="'hidden'" class="border-0 md:max-w-3xl lg:max-w-5xl xl:max-w-7xl mt-9 mx-auto w-full">
		...
		<DataTable :value="orders" :loading="orders === undefined" class="mt-4" selectionMode="single" dataKey="id">
			<Column header="Username" field="username" header-class="w-2/12" body-class="w-2/12 align-top"></Column>
			<Column header="Email" field="email" header-class="w-2/12" body-class="w-3/12 align-top"></Column>
			<Column header="Transaction ID" field="transaction_id" header-class="w-2/12" body-class="w-2/12 align-top"></Column>
			...
			<Column header="" header-class="w-3/12 ltr:text-right rtl:text-left" body-class="w-3/12 align-top ltr:text-right rtl:text-left">
				<template #body="slotProps">
					<span v-if="slotProps.data.paid_at" class=" text-create-600"><i class="pi pi-check-square ltr:mr-2 rtl:ml-2"/> {{ new Date(slotProps.data.paid_at).toLocaleString() }}</span>
					<span v-else-if="isStale(slotProps.data.created_at)" class=" text-muted-color"><i class="pi pi-bolt ltr:mr-2 rtl:ml-2"/> {{ new Date(slotProps.data.created_at).toLocaleString() }}</span>
					<span v-else class=" text-muted-color-emphasis"><i class="pi pi-bolt ltr:mr-2 rtl:ml-2"/> {{ new Date(slotProps.data.created_at).toLocaleString() }}</span>
				</template>
			</Column>
		</DataTable>
	</Panel>
...
function load() {
	WebshopService.Order.list().then((response) => {
