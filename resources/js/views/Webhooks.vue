<template>
	...
	<Panel class="border-0 max-w-5xl mx-auto mt-4">
		<p class="text-muted-color mb-6">{{ $t("webhook.description") }}</p>
		...
		<!-- Webhooks table -->
		<DataTable v-else :value="webhooks" class="w-full">
...
const eventOptions = [
	{ label: trans("webhook.event_photo_add"), value: "photo.add" as App.Enum.PhotoWebhookEventType },
	{ label: trans("webhook.event_photo_move"), value: "photo.move" as App.Enum.PhotoWebhookEventType },
	{ label: trans("webhook.event_photo_delete"), value: "photo.delete" as App.Enum.PhotoWebhookEventType },
];

const methodOptions: App.Enum.WebhookMethodType[] = ["POST", "GET", "PUT", "PATCH", "DELETE"];

const formatOptions = [
	{ label: trans("webhook.format_json"), value: "json" as App.Enum.WebhookPayloadFormatType },
	{ label: trans("webhook.format_query_string"), value: "query_string" as App.Enum.WebhookPayloadFormatType },
];

function eventLabel(event: App.Enum.PhotoWebhookEventType): string {
	return eventOptions.find((o) => o.value === event)?.label ?? event;
}

function formatLabel(format: App.Enum.WebhookPayloadFormatType): string {
