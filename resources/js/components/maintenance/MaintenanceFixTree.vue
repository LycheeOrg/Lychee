<template>
	<Card v-if="data !== undefined && fixable" class="min-h-40 shadow shadow-surface-950 rounded-lg relative bg-surface-800">
		<template #title>
		...
		<template #content>
			...
			<div class="flex gap-4 mt-1">
				<Button v-if="fixable && !loading" severity="primary" class="w-full" @click="exec">{{ $t("maintenance.fix-tree.button") }}</Button>
			</div>
		</template>
	</Card>
</template>
