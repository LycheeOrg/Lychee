			...
			<form>
				<div v-focustrap class="flex flex-col gap-4 relative w-[500px] text-sm rounded-md pt-9">
					<div class="inline-flex flex-col gap-2 px-9 w-full">
						...
						<InputText class="mx-2 !w-2/3 bg-transparent pt-1 pb-0 px-0.5 h-7
						border-b border-b-solid focus:border-b-primary-500 disabled:italic disabled:text-center inline-block"
						 :disabled="isDisabled" v-model="token" :readonly="true" />
					...
					<div class="inline w-1/4">
						...
						<a id="button_disable_token" :tile="$t('lychee.DISABLE_TOKEN_TOOLTIP')"
							@click="disableToken" :class="'cursor-pointer ' + (isDisabled ? 'hidden' : '')">
