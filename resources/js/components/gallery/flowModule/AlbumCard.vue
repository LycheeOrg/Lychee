<template>
	...
	>
		...
		<div class="p-6 bg-gradient-to-b from-surface-800 to-surface-800">
			<h3>
				<span class="text-2xl font-semibold text-surface-0">{{ props.album.title }}</span>
			</h3>
			<p v-if="!seeMore" class="text-sm text-muted-color">
				<span v-tooltip.bottom="props.album.published_created_at">{{ props.album.diff_published_created_at }}</span>
			</p>
			<template v-else>
...
const hasMore = computed(() => props.config.is_compact_mode_enabled && props.config.is_image_header_enabled && props.album.statistics !== null);
