...
export const useAlbumStore = defineStore("album-store", {
	state: () => ({
		...
		// Feature 062: tier 1 (buckets) + derived boundary metadata for the
		...
		// is the actual display source (FR-062-02/06).
	...
	actions: {
		...
		/**
		 * Flag-aware dispatcher for the subalbum-children fetch (FR-062-01):
		 ...
		 * `loadAlbums()` (FR-062-01/02/06). No windowed pagination: tier 2
		 * is whole-album-at-once (FR-062-12), so the existing
		...
		loadAlbumsV3(): Promise<void> {
			...
			return Promise.all([AlbumChildrenV3Service.getBuckets(requestedAlbumId), AlbumChildrenV3Service.getChildren(requestedAlbumId)])
				.then(([bucketsResponse, childrenResponse]) => {
					...
					// pattern — FR-062-18's intent, no separate mechanism needed).
					if (this.albumId !== requestedAlbumId) {
						return;
					}
		 ...
		 * resolve (FR-062-03) and reactively merges the combined rights into
		 * each already-adapted child in place (FR-062-04).
		 */
	...
	getters: {
		...
		// Feature 062 (FR-062-09): whether the flag-on grid/list should
		// render sticky bucket headers, or fall back to a single flat
		// unbucketed section (either tier 1 genuinely reports
		// `bucketable: false`, or FR-062-02's defensive count-mismatch
		// fallback kicked in — both collapse to the same flat rendering path).
		bucketableV3(state): boolean {
			return (state.bucketsV3?.bucketable ?? false) && state.boundariesV3 !== null;
		},
	},
});
