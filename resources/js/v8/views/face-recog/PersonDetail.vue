<template>
	<div class="h-svh overflow-y-auto">
		...
		<template v-else-if="person">
			<!-- Person header -->
			<div class="flex flex-col sm:flex-row items-center gap-6 p-6 border-b border-default">
				<div class="w-24 h-24 rounded-full overflow-hidden bg-elevated flex items-center justify-center shrink-0">
					<img
				...
				<div class="flex flex-col gap-2 text-center sm:text-left">
					...
					<UBadge v-if="!person.is_searchable" color="neutral" variant="subtle" class="w-fit mx-auto sm:mx-0">{{
						$t("people.not_searchable")
					}}</UBadge>
			...
			<!-- Batch mode info bar -->
			<div v-if="isBatchMode" class="px-6 py-2 bg-elevated flex items-center gap-3 text-sm">
				<UCheckbox :model-value="allSelected ? true : partiallySelected ? 'indeterminate' : false" @update:model-value="toggleSelectAll" />
			...
			>
				...
				<div v-if="dragVisible" class="absolute pointer-events-none border border-primary bg-primary/20 z-10" :style="dragRectStyle" />
				<div
