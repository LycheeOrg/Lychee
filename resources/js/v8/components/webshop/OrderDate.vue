<template>
	<span v-if="props.order.status === 'closed' && props.order.updated_at" class="text-success">
	...
	<span v-else-if="isStale(props.order) && props.order.created_at" class="text-muted">
		{{ new Date(props.order.created_at).toLocaleString() }}</span
	>
	<span v-else-if="props.order.created_at" class="text-highlighted">{{ new Date(props.order.created_at).toLocaleString() }}</span>
