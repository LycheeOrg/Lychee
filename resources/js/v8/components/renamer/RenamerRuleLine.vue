<template>
	<div class="flex items-center justify-between p-3 hover:bg-elevated/50 transition-colors">
		...
		<div class="flex items-center gap-2">
			<UButton v-if="canEdit" color="neutral" variant="ghost" size="sm" icon="lucide:pencil" @click="$emit('edit')" />
			<UButton v-if="canDelete" color="error" variant="ghost" size="sm" icon="lucide:trash" @click="$emit('delete')" />
		</div>
	</div>
</template>
