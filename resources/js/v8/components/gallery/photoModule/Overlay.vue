<template>
	...
	>
		<h1 class="text-xl sm:text-3xl text-neutral-200" x-text="photo.title">{{ photoStore.photo.title }}</h1>
		<p
			v-if="image_overlay_type === 'desc'"
			class="mt-1 text-base sm:text-xl text-neutral-400 prose prose-invert"
			v-html="photoStore.photo.preformatted.description"
		></p>
		<p v-if="image_overlay_type === 'date'" dir="ltr" class="mt-1 text-base sm:text-xl text-neutral-400 rtl:text-right ltr:text-left">
			<MiniIcon v-if="photoStore.photo.precomputed.is_camera_date" icon="camera-slr" class="w-4 h-4 m-0 mr-1 -mt-1 fill-neutral-400" />{{
