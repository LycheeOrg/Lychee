<template>
	<USlideover v-model:open="is_metrics_open" :dismissible="true" side="right" class="w-sm">
		<template #header>
			...
			<div class="flex flex-col">
				...
				<div v-for="item in prettifiedData" :key="item.action + item.ago" class="flex pt-2 pb-1">
					...
					<div class="w-12 h-12 relative shrink-0">
						<div
							class="absolute top-0 right-0 translate-x-2.5 -translate-y-2 w-6 h-6 rounded-full bg-primary flex justify-center items-center border-3 border-solid border-default text-inverted"
						>
							<UIcon
								class="text-2xs"
								:name="
...
function printSingular(data: LiveMetrics) {
	const visitor = `<span class="font-bold text-highlighted">${trans("statistics.metrics.a_visitor")}</span>`;
	switch (data.action) {
		case "visit":
...
function printPlural(data: LiveMetrics) {
	const visitors = sprintf(`<span class="font-bold text-highlighted">${trans("statistics.metrics.visitors")}</span>`, data.count);
	switch (data.action) {
		case "visit":
