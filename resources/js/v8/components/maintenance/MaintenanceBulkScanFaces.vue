<template>
	<UCard v-if="data !== undefined && data !== 0" class="min-h-40 relative bg-muted/50">
		...
		<template #footer>
			<UButton v-if="data !== 0 && !loading" color="primary" variant="solid" class="w-full justify-center" @click="exec">
				{{ $t("people.scan_faces") }}
			</UButton>
		</template>
	</UCard>
...
const description = computed(() => {
	...
	return sprintf(trans("maintenance.bulk-scan-faces.description"), data.value);
});

function load() {
	loading.value = true;
	MaintenanceService.bulkScanFacesCheck()
		.then((response) => {
...
function exec() {
	loading.value = true;
	MaintenanceService.bulkScanFacesDo()
		.then(() => {
