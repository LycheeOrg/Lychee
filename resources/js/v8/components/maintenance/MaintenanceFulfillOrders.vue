<template>
	<MaintenanceRow v-if="data !== undefined && data > 0">
		<template #title>{{ $t("maintenance.fulfill-orders.title") }}</template>
		...
		<template #actions>
			<UButton variant="soft" v-if="data > 0 && !loading" color="primary" @click="exec">
				{{ $t("maintenance.fulfill-orders.button") }}
			</UButton>
		</template>
	</MaintenanceRow>
</template>
...
const description = computed(() => {
	return sprintf(trans("maintenance.fulfill-orders.description"), data.value);
});
