<template>
	<UCard v-if="contactConfig?.is_contact_form_enabled" class="contact-form">
		...
		<form v-else class="flex flex-col" @submit.prevent="submit">
			...
			<UFormField v-if="contactConfig.security_question" class="mb-4">
				<template #label
...
const emit = defineEmits<{
	loaded: [config: App.Http.Resources.GalleryConfigs.ContactConfig];
}>();
