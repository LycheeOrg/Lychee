...
|------------|------|--------|----------|----------|---------|---------|----------|
| 054 | Configurable Landing Page | Planning | P2 | LycheeOrg | 2026-08-10 | 2026-08-10 | Spec, plan, tasks drafted, then extended across six same-day revision rounds. 19 questions resolved (Q-054-01..19), all with full Decision Cards in spec.md Appendix per `spec-guidelines/open-questions-format.md`: 4 named layouts (classic/portfolio/minimal/studio), v8-only, SE-gating, extra links, dedicated `LandingConfig.vue` admin page (mirrors Feature 045's `NsfwConfig.vue`) absorbing the **entire** pre-existing `Mod Welcome` category across 3 tabs (Settings/Links/Featured) with that category filtered from the flat list, landing-specific custom CSS dropped for the existing global mechanism, featured content redesigned as automatic-default + full manual photo/album curation (new `LandingFeaturedItem` CRUD), plus 9 review-found gaps fixed (Q-054-11..19): `landing_show_stats` corrected to `portfolio`-only, confirmed the Featured-tab picker already surfaces private content to admins, SE-badge dropdown behaviour decided, a from-scratch `Reorder` endpoint contract designed (no prior art existed in this codebase), route-registration citation corrected to cover both `paths.ts` and `routes.ts`, `Mod Welcome` filter citation pinned to `SettingsController.php:83`, missing `icon` validation added. ~55 tasks across 16 increments. Not yet implemented. |
