...
|------------|------|--------|----------|----------|---------|---------|----------|
| 054 | Configurable Landing Page | Planning | P2 | LycheeOrg | 2026-08-10 | 2026-08-10 | Spec, plan, tasks drafted. 4 named layouts (`classic` free/default, `portfolio`/`minimal`/`studio` SE-gated) selectable via `landing_layout`; configurable hero text position, animation preset, CTA text, about block; admin-managed extra links (`LandingLink`, no icon); featured content on `portfolio` (automatic — latest public albums — or fully manual curation of specific photos/albums via `LandingFeaturedItem`); Contact-form surfacing on `portfolio`/`minimal`. New admin page `LandingConfig.vue` (Settings tab with a live, instantly-reactive preview mirroring the Watermarker module's `WatermarkPreview.vue`, plus Links and Featured tabs) — its 10 settings also remain fully visible in the flat generic Settings list. 63 tasks across 12 increments (with 4 sub-increments for parallel CRUD/refactor work). Not yet implemented. |
