...
|------------|------|--------|----------|----------|---------|---------|----------|
| 050 | Album Tags | Planning | P2 | LycheeOrg | 2026-07-12 | 2026-07-12 | Spec, plan, tasks drafted; analysis gate passed. All 3 open questions resolved (Q-050-01 A, Q-050-02 A, Q-050-03 A). 20 tasks across 10 increments. Adds shared `Tag` support to regular `Album`s (new `albums_tags` pivot, `Album::tags()`/`Tag::albums()` relations) — unified with the existing photo/tag-album `Tag` vocabulary. Surfaces on `/tag/{id}` (new Albums section, Q-050-01 A), `Search` (`tag:` modifier + plain-text, Album-only — never `TagAlbum`, NFR-050-01), and `/tags` (split `num_photos`/`num_albums` counts, album-only tags now visible, Q-050-02 A). `TagCleanupTrait`/`MergeTag`/`DeleteTag` extended so album-only tags are never silently purged. v8-only (no v7 changes, NFR-050-02). No transitive photo↔album-tag relationship (explicit non-goal). |
