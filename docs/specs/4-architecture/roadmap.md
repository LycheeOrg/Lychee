...
|------------|------|--------|----------|----------|---------|---------|----------|
| 052 | Managed Cache Service | Planning | P2 | User | 2026-07-21 | 2026-07-21 | Spec drafted and Q-052-01..05 all resolved (Option A each). Generic `App\Services\Cache\ManagedCacheService` (`remember`/`forgetTag`, hand-rolled key-list tag bookkeeping since default `file` cache driver has no native tags) proven via two pilot consumers: `AlbumRepository::getChildrenPaginated()` (sub-albums) and `PhotoRepository::getPhotosForAlbumPaginated()` (photos) — both permission-filtered, hit on every album-view page load, and already listed (uncached-by-default) in the existing HTTP response cache's route table. Fixes three confirmed invalidation gaps: `Actions\Album\Move::do()`, `SharingController` (create/edit/delete/propagate), and `UserGroupsManagementController` (addUser/removeUser/updateUserRole) all dispatched no event previously. Invalidation listener also evicts a changed album's immediate-parent tag (closes the "negative cache" gap for a child becoming newly visible/hidden). New config `managed_cache_enabled` (default true) + `managed_cache_ttl`. Ancestor-path tagging (`Album::ancestorsOf()`) makes ancestor permission changes cascade to descendants without a runtime tree walk. Next: plan.md/tasks.md. |
...
*Last updated: 2026-07-21 (Feature 052 spec drafted — Managed Cache Service, Q-052-01..05 resolved)*
