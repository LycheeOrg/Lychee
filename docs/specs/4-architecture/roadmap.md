...
|------------|------|--------|----------|----------|---------|---------|----------|
| 053 | Memory Profiler | Planning | P3 | User | 2026-07-28 | 2026-07-28 | Spec/plan/tasks drafted; all 4 open questions resolved same day (Q-053-01..04, all Option A). Corrects the initial brief: `arnaud-lb/php-memory-profiler` is a PECL extension (`memprof_enable/disable/dump_pprof`), not the OOP Composer snippet supplied. Global terminable middleware captures a pprof dump per request into a new `profiling` local disk (`storage/profiling`), gated by `MEMORY_PROFILER_ENABLED` (features.php pattern), always-on while enabled (Q-053-04), count-based retention via `MEMORY_PROFILER_MAX_TRACES` (Q-053-03). New owner-only Blade admin page `/admin/profiler` (new `owner` route middleware, reusable) renders SVG call-graphs via `google-pprof` (+ Graphviz `dot`) — both now bundled into the production `Dockerfile` (Q-053-02 modified); `memprof` itself stays a manually-installed, documented opt-in extension. Default production runtime is Octane/FrankenPHP; `memprof`'s per-request semantics under that model are unverified — shipping anyway with a risk banner + manual validation task per **ADR-0008** (Q-053-01). Ready for the analysis gate; no implementation started yet. |
| 051 | v8 Admin Setup Page | Testing | P2 | User | 2026-07-26 | 2026-07-26 | Implementation complete (T-051-01..12,15). New `CreateInitialAdmin` action shared by legacy Blade `SetUpAdminController` and new `AdminSetupController` (`POST /Admin::Setup`); `GET /setup-admin` route + `ToAdminSetter` branch on `nuxt_ui` flag (ADR-0007); v8 `AdminSetupPage.vue` + `admin-setup-service.ts`; `admin-setup` route added to shared `paths.ts` with v7 `Placeholder.vue` fallback; 22-locale translations. `php artisan test`: all green (incl. new `CreateInitialAdminTest`, `AdminSetupTest` x2). `make phpstan`: 0 errors on touched files. `npm run check`: clean. Q-051-05 (no JS test runner in this repo) resolved by the user (Option A — accept the gap, no dependency added). Manual browser verification not performed this session, to avoid mutating the dev environment; HTTP-level behaviour covered by feature tests instead. |
