...
- `resources/js/stores/UserState.ts` (`useUserStore()`) — `user.id`/`isGuest`/`isAdmin`, needed for FR-062-04's combination formula. `may_upload` is not exposed here — `albumsStore.rootRights?.can_upload` is used instead (`UserResource` never surfaces the underlying column to the frontend).
...
- `resources/js/v8/components/gallery/albumModule/AlbumNavTree.vue` — existing `@tanstack/vue-virtual` usage in this codebase; a general convention reference for DO-062-03/04 (`translate3d` positioning, single spacer sized to `getTotalSize()`, `contain` hints), not a template whose internals must be replicated exactly — besides the multi-column/sticky-header/aspect-ratio-height requirements, it also scrolls inside its own fixed-height sidebar box (`useVirtualizer`), whereas DO-062-03/04 share `AlbumPanel.vue`'s ordinary page/window scroll (`useWindowVirtualizer` with a `scrollMargin` tracking the grid's own document-relative top offset). Also the existing precedent for `<Thumb>` usage that DO-062-06 mirrors.
...
- `resources/js/services/album-service.ts`'s `AlbumService`/`axios-cache-interceptor` convention — reused directly (not mirrored) by FR-062-19's new `AlbumChildrenV3Service`; `AlbumService.clearCache()` was extended to also clear the three new v3 cache entries (Q-062-12 Option B).
...
5. **I5 – Grid virtualization**
   - _Goal:_ `AlbumThumbGridVirtual.vue` (DO-062-03), wired into `AlbumPanel.vue` in place of `AlbumThumbPanel.vue`+`<Pagination>` when the flag is on (via a small `AlbumThumbPanelVirtual.vue` grid/list dispatcher), rendering I3's `AlbumThumbVirtual.vue` per child.
