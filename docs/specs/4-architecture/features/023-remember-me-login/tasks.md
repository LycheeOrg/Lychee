...
- [ ] T-023-01 – Add `REMEMBER_ME_ATTRIBUTE = 'remember_me'` constant to `RequestAttribute` (FR-023-06).
...
- [ ] T-023-02 – Update `LoginRequest` to validate optional `remember_me` boolean field (FR-023-01, S-023-05, S-023-11).
...
- [ ] T-023-03 – Write failing tests for remember-me login behavior (FR-023-01, FR-023-04, S-023-01, S-023-02, S-023-05, S-023-06, S-023-11, NFR-023-03).
...
- [ ] T-023-04 – Update `AuthController::login()` to pass `remember` flag to auth calls (FR-023-04, S-023-02, S-023-07, S-023-08).
...
- [ ] T-023-05 – Update login log messages to include remember flag (FR-023-04).
  _Intent:_ Append `[remember=true/false]` to existing log messages in `AuthController::login()` and `attemptLdapLogin()`.
  _Verification commands:_
  - `make phpstan`
  - `vendor/bin/php-cs-fixer fix --dry-run`

- [ ] T-023-06 – Write test: logout after remember-me login clears cookie and rotates token (FR-023-05, S-023-04).
...
- [ ] T-023-07 – Green: all backend tests pass including new remember-me tests (NFR-023-03, NFR-023-04).
...
- [ ] T-023-08 – Update `auth-service.ts` to accept and send `remember_me` parameter (FR-023-03).
...
- [ ] T-023-09 – Add "Remember Me" checkbox to `LoginForm.vue` (FR-023-02, UI-023-01, UI-023-02, UI-023-03, S-023-09, S-023-10).
...
- [ ] T-023-10 – Green: frontend checks pass (FR-023-02, FR-023-03).
...
- [ ] T-023-11 – Add English translation string for "Remember me" (NFR-023-05).
...
- [ ] T-023-12 – Add placeholder translations for 21 other languages (NFR-023-05).
...
- [ ] T-023-13 – End-to-end test: remember cookie re-authenticates after session flush (S-023-03).
...
- [ ] T-023-14 – End-to-end test: logout invalidates remember cookie (S-023-04).
  _Intent:_ Login with `remember_me = true` → logout → attempt request with old remember cookie → verify 401.
  _Verification commands:_
  - `XDEBUG_MODE=off php artisan test --filter=<TestClass>`

- [ ] T-023-15 – Run full quality gate (all scenarios).
...
- [ ] T-023-16 – Update knowledge map with remember-me login flow.
...
- [ ] T-023-17 – Update roadmap with Feature 023 completion status.
