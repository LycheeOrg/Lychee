...
- [ ] T-062-00 – Verify no v8 frontend code references the old `/children` paths before renaming (F-062-01, Q-062-12).
...
- [ ] T-062-01 – Create `App\Http\Controllers\Gallery\AlbumListing` namespace/folder (F-062-12).
...
- [ ] T-062-03 – Widen `AlbumChildrenDataResource` with `owner_ids[]` (F-062-06, DO-062-03).
  _Intent:_ Additive field, populated for both sub-album and root tiers (data already selected in the existing query — free).
  _Verification commands:_
  - `php artisan test --filter=AlbumChildrenDataV3Test`

- [ ] T-062-04 – Widen `AlbumChildrenRightsResource`'s `owner_id` to nullable, omitted from the payload when null (F-062-06, DO-062-04, Q-062-16).
...
- [ ] T-062-05 – Write failing migration test (F-062-08, S-062-13, FX-062-02).
...
- [ ] T-062-06 – Remove `ColumnSortingAlbumType::OWNER_ID` (F-062-08, G5).
...
- [ ] T-062-07 – Write the migration (`UPDATE albums`/`UPDATE configs`, mirrors `fix_sorting_albums.php`) (F-062-08, NFR-062-07).
...
- [ ] T-062-08 – Verify no other code pattern-matches `ColumnSortingAlbumType::OWNER_ID` by name (F-062-08).
...
- [ ] T-062-09 – Verify/regenerate frontend types and confirm no manual dropdown edit is needed (F-062-08, Test Strategy).
...
- [ ] T-062-10 – Write failing `RecomputeRootAlbumBucketsJobTest` (F-062-07, S-062-10/11).
...
- [ ] T-062-11 – Implement `RecomputeRootAlbumBucketsJob` (F-062-07, NFR-062-03).
...
- [ ] T-062-12 – Dispatch from `SettingsController::setConfigs()` on the 5-key intersection; extend `ALBUM_LISTING_COARSE_FLUSH_CONFIGS` (F-062-07, S-062-10/11/12).
...
- [ ] T-062-13 – New `GetScopedAlbumsRequest` with `scope` validation (F-062-02, S-062-01..04/18).
...
- [ ] T-062-14 – Implement `AlbumRootController::index()`/`buckets()` for `scope=own` (F-062-03, S-062-05).
...
- [ ] T-062-15 – Implement `AlbumRootController::index()`/`buckets()` for `scope=shared` (F-062-04/05, S-062-06/07/29).
...
- [ ] T-062-16 – Implement `AlbumRootController::rights()` (F-062-06, S-062-08/09).
...
- [ ] T-062-17 – New `CacheKeyProvider` methods for the root tier, keyed by `(scope, user_id)` (F-062-13, NFR-062-08).
...
- [ ] T-062-18 – Register `/Albums/root`, `/Albums/root/buckets`, `/Albums/root/rights` ahead of `{album_id}` family (F-062-01).
...
- [ ] T-062-19 – `deduplicate_pinned_albums` parity check (S-062-20).
  _Intent:_ Behaviour identical to v2 for both scopes.
  _Verification commands:_
  - `php artisan test --filter=AlbumRootV3Test`

- [ ] T-062-19a – v2 parity fixture: `own ∪ shared` reconstructs `Top::queryRootAlbums()` exactly (F-062-03/04, NFR-062-05).
...
- [ ] T-062-20 – New `GetAlbumCategoryRequest` (feature-flag gate only) (F-062-01, S-062-18).
...
- [ ] T-062-21 – New `AlbumCategoryListResource` (F-062-09, DO-062-05).
...
- [ ] T-062-22 – Implement `AlbumCategoryController::smart()` (F-062-09, S-062-14).
...
- [ ] T-062-23 – Implement `AlbumCategoryController::tags()` (F-062-09, S-062-15).
...
- [ ] T-062-23a – Implement `AlbumCategoryController::pinned()`, scope-aware (F-062-15, S-062-15/22/23/24/25).
...
- [ ] T-062-23c – Implement `AlbumCategoryController::persons()`, scope-aware (F-062-15, S-062-15/26/27).
...
- [ ] T-062-23b – Widen `pinnedAlbumsListingKey()` and `personAlbumsListingKey()` with a `scope` argument (F-062-13, S-062-23/26).
...
- [ ] T-062-24 – New `AlbumCategoryRightsResource` + `AlbumCategoryController::tagsRights()` (F-062-10, DO-062-06, S-062-16).
  _Intent:_ `ids[]`/`grants_edit[]`/`grants_download[]`/`grants_delete[]`; admin short-circuit.
  _Verification commands:_
  - `php artisan test --filter=AlbumCategoryV3Test`

- [ ] T-062-25 – Register `/Albums/smart`, `/Albums/persons`, `/Albums/tags`, `/Albums/tags/rights`, `/Albums/pinned` ahead of `{album_id}` family (F-062-01).
  _Intent:_ Avoid route-parameter shadowing.
  _Verification commands:_
  - `php artisan route:list --path=Albums`

- [ ] T-062-26 – Per-category cache-invalidation independence check (S-062-17).
...
- [ ] T-062-26a – Verify cache-invalidation coverage directly, not by inspection (F-062-14).
...
- [ ] T-062-27 – Implementation Drift Gate: confirm `routes/api_v2.php`, `AlbumsController.php`, `Top.php`, `RootAlbumResource.php`, `resources/js/v7/**` unchanged (NFR-062-04, S-062-19).
  _Verification commands:_
...
- [ ] T-062-28 – Full new-surface + targeted regression test run (S-062-01..29).
  _Verification commands:_
  - `php artisan test --filter=AlbumRootV3Test`
  - `php artisan test --filter=AlbumCategoryV3Test`
  - `php artisan test --filter=AlbumBucketsV3Test`
...
- [ ] T-062-29 – File-count check against NFR-062-09.
...
- [ ] T-062-30 – Quality gates.
  _Verification commands:_
  - `make phpstan`
  - `vendor/bin/php-cs-fixer fix --dry-run --diff`

- [ ] T-062-31 – Documentation updates.
...
- [ ] T-062-32 – Deployer-facing note about `lychee:recompute-album-buckets` re-run post-migration (F-062-08 note).
