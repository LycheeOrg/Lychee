...
| Owners | LycheeOrg |
| Linked plan | `docs/specs/4-architecture/features/052-managed-cache-service/plan.md` (not yet created) |
| Linked tasks | `docs/specs/4-architecture/features/052-managed-cache-service/tasks.md` (not yet created) |
| Roadmap entry | #052 |
...
|----|-------|---------------------------|
| UI-052-01 | `managed_cache_enabled` toggle in Settings | Admin views/edits the toggle in the existing Settings page (same `BoolField` pattern as `cache_enabled`); default `true`. |
| UI-052-02 | `managed_cache_ttl` numeric field in Settings | Admin views/edits the default TTL (seconds); default `3600`. |
