...
2. **I2 – `TitleSplitter` service + explicit write-site wiring**
   ...
   - _Steps:_ Write `TitleSplitter::split()` (unit-tested first — trailing digits, parenthesised fallback, no-digit fallback, case-fold, >19-digit truncation, empty string, unicode, **extension-suffix cases** `xxx_123.jpg`/`xxx (123).xts`/`xxx.2`/`Vol.II` per Q-060-03/spec Appendix worked examples). Then add an explicit `TitleSplitter::split()` call + `title_base`/`title_index` assignment at each of the 12 identified write sites, immediately after `title` is set and before `save()`/batch-update:
