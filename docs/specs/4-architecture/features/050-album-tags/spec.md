...
| FR-050-02 | `UpdateAlbumRequest` (`PATCH /Album`) accepts an optional `tags: string[]` field, validated `sometimes\|array` (**not** `present`) — the key may be entirely absent from the payload, in which case existing album tags are left untouched. This is required because the legacy v7 (PrimeVue) frontend calls this same endpoint and never sends a `tags` key; treating an absent key the same as an empty array would silently wipe any tags set from v8. `UpdateAlbumRequest::tagsProvided()` exposes whether the key was present. When present (empty array or not), `AlbumController::updateAlbum` resolves tag names via `Tag::from()` (find-or-create, case-insensitive) and calls `$album->tags()->sync(...)`, mirroring `AlbumController::updateTagAlbum`'s existing `Tag::from($request->tags()); $album->tags()->sync(...)` pattern. | Album's tag set is replaced with the submitted set when `tags` is present (including clearing all tags via `tags: []`); unknown names become new `Tag` rows; existing case-insensitive matches are reused; omitting the key entirely leaves the album's existing tags untouched. | `tags.*` validated `required|string|min:1` (same rule as `SetPhotosTagsRequest`); authorization unchanged (`AlbumPolicy::CAN_EDIT`, already enforced by `UpdateAlbumRequest::authorize()`). | Validation errors return 422 as with any other `UpdateAlbumRequest` field. | None. | Mirrors FR pattern of `UpdateTagAlbumRequest`/`SetPhotosTagsRequest`; the optional/`tagsProvided()` refinement was discovered during implementation once v7's `AlbumProperties.vue` (which also calls `PATCH /Album` and predates this feature) was found to never send a `tags` key. |
