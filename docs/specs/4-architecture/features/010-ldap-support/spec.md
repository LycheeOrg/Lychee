...
| FR-010-04 | User auto-provisioning | When LDAP user authenticates for first time and auto-provision enabled, create new User record with LDAP attributes and set `is_ldap=true` | Check if username already exists; validate LDAP attributes meet User model constraints | If auto-provision disabled and user doesn't exist, reject login with appropriate error | Log `ldap.user.created` for new users | Just-in-time provisioning pattern |
| FR-010-08 | LDAP user differentiation | LDAP-created users have `is_ldap=true` flag; exposed via UserResource to frontend; profile UI disables username/password editing for LDAP users with message "User login information are LDAP managed" | Validate user update requests reject username/password changes when `is_ldap=true`; frontend checks UserResource.is_ldap flag | If attempt to change LDAP user credentials, return validation error | Log validation rejections for LDAP user credential changes | Security: prevent LDAP user credential hijacking |
...
| S-010-11 | LDAP user tries to change password in profile UI → username/password fields disabled, message shown |
| S-010-12 | Admin tries to update LDAP user credentials via user management → validation error returned |
| S-010-13 | Local (non-LDAP) user edits profile → username/password fields editable normally |
...
- **Core:** 
  ...
  - Test `is_ldap` flag set correctly during provisioning

- **Application:** 
  - Integration tests for user provisioning flow with `is_ldap=true`
  ...
  - Test validation rejection when attempting to update LDAP user credentials
...
- **REST:** 
  ...
  - Test `/api/Auth::user` returns `is_ldap` flag in UserResource
  - Test `/api/UserManagement` rejects username/password updates for LDAP users
...
- **UI (JS/Selenium):** 
  ...
  - Test profile page disables username/password fields for LDAP users
  - Test profile page shows "User login information are LDAP managed" message

- **Docs/Contracts:** 
  - Document all .env variables in `.env.example`
  - Update authentication documentation with LDAP setup guide
  - Document `is_ldap` flag in UserResource TypeScript types
...
| UI-010-04 | Profile page (LDAP user) | LDAP user views profile page → username and password fields disabled with message "User login information are LDAP managed" |
| UI-010-05 | Profile page (local user) | Non-LDAP user views profile page → username and password fields editable as normal |
...
| COL-010-02 | users | is_ldap | boolean | default: false | Flag indicating whether user was created/managed via LDAP authentication; prevents manual password/username changes for LDAP users |

**Migrations:**
- `2026_01_25_add_display_name_to_users_table.php`
- `2026_01_26_add_is_ldap_to_users_table.php`

```php
// Migration 1: display_name
...
- `is_ldap` flag set to true when user auto-provisioned from LDAP; prevents username/password editing in profile UI and validation in user update requests
...
3. **Certificate validat6on:** Verify TLS certificates unless using self-signed certs in dev (set `LDAP_TLS_VERIFY_PEER=false` only in dev)
