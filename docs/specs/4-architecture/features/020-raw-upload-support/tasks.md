...
- [ ] T-020-01 – Write failing test: `SizeVariantType::RAW->value === 0` and `ORIGINAL->value === 1` (FR-020-01, S-020-13).
...
- [ ] T-020-02 – Update `SizeVariantType` enum: `RAW = 0`, shift all others by +1 (`ORIGINAL = 1` … `PLACEHOLDER = 8`) (FR-020-01).
...
- [ ] T-020-03 – Fix all `SizeVariantType` exhaustive match arms across codebase (FR-020-01).
...
- [ ] T-020-04 – Create two-phase migration for `type` column shift (FR-020-02, NFR-020-01, NFR-020-05).
...
- [ ] T-020-05 – Update `SizeVariants` model: add `$raw` property, update `add()`, `getSizeVariant()`, `toCollection()`, `deleteAll()`, add `getRaw()` (FR-020-11, S-020-17).
  _Intent:_ Model CRUD for RAW variant. `deleteAll()` must also delete RAW file from storage.
  _Verification commands:_
  - `make phpstan`
  - `XDEBUG_MODE=off php artisan test --no-coverage`

- [ ] T-020-06 – Update `SizeVariantFactory` (test factory) with new integer values (FR-020-15).
  _Intent:_ Test factories must produce correct `type` values for all size variants.
  _Verification commands:_
  - `XDEBUG_MODE=off php artisan test --no-coverage`

- [ ] T-020-07 – Green: all existing tests pass with new enum values (FR-020-01, S-020-13).
...
- [ ] T-020-08 – Write failing test: ORIGINAL with `short_path` ending `.nef` → after migration becomes `type = RAW (0)` (FR-020-16, S-020-23).
  _Intent:_ Red test for reclassification migration.
  _Verification commands:_
  - `XDEBUG_MODE=off php artisan test --filter=<TestClass>`

- [ ] T-020-09 – Write failing test: ORIGINAL with `short_path` ending `.pdf` → stays `type = ORIGINAL (1)` after migration (FR-020-16, S-020-22).
  _Intent:_ Red test confirming PDF exclusion.
  _Verification commands:_
  - `XDEBUG_MODE=off php artisan test --filter=<TestClass>`

- [ ] T-020-10 – Create reclassification migration: read `raw_formats` config, match extension from `short_path`, set `type = 0` for matching rows (excluding `.pdf`) (FR-020-16).
...
- [ ] T-020-11 – Green: reclassification tests pass (FR-020-16, S-020-23).
...
- [ ] T-020-12 – Add `DownloadVariantType::RAW = 'RAW'` case and update `getSizeVariantType()` mapping (FR-020-06, DO-020-02).
  _Intent:_ Enum extension for download.
  _Verification commands:_
  - `make phpstan`

- [ ] T-020-13 – Verify `PurchasableSizeVariantType` has no RAW case (FR-020-10, S-020-12).
...
- [ ] T-020-14 – Update `BaseArchive::extractFileInfo()` to handle RAW variant (FR-020-06, S-020-16).
...
- [ ] T-020-15 – Write test: `raw_download_enabled` config default is `false` (FR-020-05, CFG-020-01).
  _Intent:_ Red test for config key existence and default.
  _Verification commands:_
  - `XDEBUG_MODE=off php artisan test --filter=<TestClass>`

- [ ] T-020-16 – Create migration inserting `raw_download_enabled` config row (boolean, default `0`, category 'Image Processing') (FR-020-05, CFG-020-01).
  _Intent:_ Config infrastructure for RAW download gating.
  _Verification commands:_
  - `make phpstan`
  - `XDEBUG_MODE=off php artisan test --no-coverage`

- [ ] T-020-17 – Add English translation strings for `raw_download_enabled` setting + placeholders for 21 languages (CFG-020-01, UI-020-03).
...
- [ ] T-020-18 – Define `ConvertibleRawFormat` enum/constant list with all extensions from FR-020-09 (FR-020-09).
  _Intent:_ Central definition of extensions that trigger RAW+convert pipeline (includes `.heic`, `.heif`).
  _Verification commands:_
  - `make phpstan`

- [ ] T-020-19 – Write failing tests for RAW upload pipeline (FR-020-03, FR-020-04, S-020-01, S-020-02, S-020-03, S-020-04, S-020-05, S-020-14, S-020-21, S-020-22, S-020-24).
...
- [ ] T-020-20 – Create `RawToJpeg` converter class (FR-020-04, FR-020-09).
...
- [ ] T-020-21 – Create `DetectAndStoreRaw` Init pipe replacing `ConvertUnsupportedMedia` (FR-020-03, FR-020-04).
...
- [ ] T-020-22 – Create `CreateRawSizeVariant` Standalone pipe (FR-020-03).
  _Intent:_ Creates RAW `size_variant` DB row if raw path is set in DTO. Placed after `CreateOriginalSizeVariant` in pipeline.
  _Verification commands:_
  - `make phpstan`

- [ ] T-020-23 – Update `Create.php` pipe chain: replace `ConvertUnsupportedMedia` with `DetectAndStoreRaw`, add `CreateRawSizeVariant` after `CreateOriginalSizeVariant` (FR-020-03, FR-020-04).
  _Intent:_ Wire new pipes into upload pipeline.
  _Verification commands:_
  - `make phpstan`

- [ ] T-020-24 – Remove `HeifToJpeg`, `ConvertUnsupportedMedia`, `PhotoConverterFactory`, `ConvertableImageType` (FR-020-04).
...
- [ ] T-020-25 – Green: all RAW upload tests pass (FR-020-03, FR-020-04, S-020-01 through S-020-05, S-020-14, S-020-21, S-020-22, S-020-24).
...
- [ ] T-020-26 – Write failing test: `Watermarker::do()` on RAW size variant returns early without watermarking (FR-020-07, S-020-11, S-020-15).
  _Intent:_ Red test for watermark skip.
  _Verification commands:_
  - `XDEBUG_MODE=off php artisan test --filter=<TestClass>`

- [ ] T-020-27 – Add RAW to early-return guard in `Watermarker::do()` alongside PLACEHOLDER skip (FR-020-07).
...
- [ ] T-020-28 – Write failing API tests: `has_raw: true` when RAW row exists, `has_raw: false` otherwise (FR-020-08, S-020-09, S-020-10).
...
- [ ] T-020-29 – Add `public bool $has_raw` to `PhotoResource`, derived from `$photo->size_variants->getRaw() !== null` (FR-020-08, API-020-02).
...
- [ ] T-020-30 – Regenerate TypeScript types (FR-020-08).
...
- [ ] T-020-31 – Write failing test: RAW download with config disabled → 403 (FR-020-06, S-020-07).
  _Intent:_ Red test for download gating.
  _Verification commands:_
  - `XDEBUG_MODE=off php artisan test --filter=<TestClass>`

- [ ] T-020-32 – Write failing test: RAW download with config enabled → 200 with file (FR-020-06, S-020-06).
  _Intent:_ Red test for successful RAW download.
  _Verification commands:_
  - `XDEBUG_MODE=off php artisan test --filter=<TestClass>`

- [ ] T-020-33 – Write failing test: RAW download when photo has no RAW → 404 (S-020-08).
  _Intent:_ Red test for missing-variant case.
  _Verification commands:_
  - `XDEBUG_MODE=off php artisan test --filter=<TestClass>`

- [ ] T-020-34 – Write failing test: ZIP download includes RAW when enabled (S-020-16).
  _Intent:_ Red test for bulk download behavior.
  _Verification commands:_
  - `XDEBUG_MODE=off php artisan test --filter=<TestClass>`

- [ ] T-020-35 – Update download controller/request validation to check `raw_download_enabled` when variant is RAW (FR-020-06, API-020-01).
...
- [ ] T-020-36 – Audit diagnostics checks for hardcoded type values: `PlaceholderExistsCheck`, `SmallMediumExistsCheck`, `WatermarkerEnabledCheck` (FR-020-14, S-020-19).
...
- [ ] T-020-37 – Audit `Spaces.php` and `RSS/Generate.php` for type value references (FR-020-14, S-020-20).
...
- [ ] T-020-38 – Update TypeScript types if not already current from I7 (FR-020-08, FR-020-12).
  _Intent:_ Ensure `has_raw` and `RAW` variant type exist in TS types.
  _Verification commands:_
  - `npm run check`

- [ ] T-020-39 – Add conditional "Download RAW" button in photo detail view download section (FR-020-12, UI-020-01, UI-020-02, S-020-18).
...
- [ ] T-020-40 – Add English translation strings for "Download RAW" button + placeholders for 21 languages (FR-020-12, UI-020-01).
  _Intent:_ Only modify PHP arrays in `lang/<locale>/*.php`. Never modify `lang/php_*.json` files.
  _Verification commands:_
  - `npm run format`
  - `npm run check`

- [ ] T-020-41 – Verify settings UI shows `raw_download_enabled` toggle (UI-020-03).
...
- [ ] T-020-42 – End-to-end test: upload RAW → verify RAW + ORIGINAL + thumbnails → download RAW (all scenarios).
  _Intent:_ Full pipeline integration test.
  _Verification commands:_
  - `XDEBUG_MODE=off php artisan test --filter=<TestClass>`

- [ ] T-020-43 – End-to-end test: upload HEIC → verify RAW (HEIC) + ORIGINAL (JPEG) → download RAW (S-020-02).
  _Intent:_ HEIC-specific integration test using existing fixtures.
  _Verification commands:_
  - `XDEBUG_MODE=off php artisan test --filter=<TestClass>`

- [ ] T-020-44 – Verify standard JPEG/PNG upload is completely unaffected (S-020-04).
  _Intent:_ Regression check — no behavioral change for normal uploads.
  _Verification commands:_
  - `XDEBUG_MODE=off php artisan test --no-coverage`

- [ ] T-020-45 – Run full quality gate (all file types modified).
...
- [ ] T-020-46 – Update knowledge map with RAW size variant type and conversion pipeline.
...
- [ ] T-020-47 – Update image-processing reference doc.
