...
| FR-030-47 | **Wire request authorizers to per-resource gates.** All existing `Request` classes that currently bypass per-resource ownership checks for face operations (those carrying `// TODO: Make sure FacePermissionMode applies here` or equivalent gaps) shall be updated to use the new `PhotoPolicy` / `AlbumPolicy` gate constants. Specifically: (a) `AssignFaceRequest::authorize()` → `Gate::check(PhotoPolicy::CAN_ASSIGN_FACE_ON_PHOTO, $face->photo)`; (b) `ToggleDismissedRequest::authorize()` → `Gate::check(PhotoPolicy::CAN_DISMISS_FACE, $face->photo)` (removes the inline ownership check and the `// TODO`); (c) `BatchFaceRequest::authorize()` → `Gate::check(PhotoPolicy::CAN_ASSIGN_FACE_ON_PHOTO, $photo)` for **every** photo carrying an explicitly selected object (the photo of each resolved `face_ids` entry, or each resolved `photo_ids` entry), whether or not `album_id` is supplied. When `album_id` **is** supplied it acts as an **additional** constraint on top of those per-object checks: `Gate::check(AlbumPolicy::CAN_BATCH_FACE_OPS, $album)` must pass **and** every selected photo must be linked to that album (see FR-030-48); (d) `ScanPhotosRequest::authorize()` → when **no** `photo_ids` are given (album-wide scan): `Gate::check(AlbumPolicy::CAN_TRIGGER_SCAN_ON_ALBUM, $this->album)`; whenever `photo_ids` **are** given: `Gate::check(PhotoPolicy::CAN_TRIGGER_SCAN_ON_PHOTO, $photo)` for each resolved photo, plus — when `album_id` is supplied alongside them — the album gate **and** album membership of every selected photo (see FR-030-48); (e) `GetAlbumPersonsRequest::authorize()` → existing album-access check **AND** `Gate::check(AlbumPolicy::CAN_VIEW_ALBUM_PEOPLE, $album)`; (f) `PhotoResource::buildFaceData()` → `Gate::check(PhotoPolicy::CAN_VIEW_FACE_OVERLAYS, $photo)` replacing the inline mode-check block. This resolves Q-030-63 and Q-030-72. | Each request returns 403 when the calling user does not have ownership of the relevant photo or album in privacy-preserving/restricted modes. | — | 403 returned for unauthorized callers; no silent data leak. | — | Owner directive, Q-030-63, Q-030-72, Q-030-79 |
| FR-030-48 | **Bind a supplied `album_id` to the objects selected by ID (GHSA-x6f7-qp5q-w37f).** On `POST /Face/batch` and `POST /FaceDetection/scan`, an `album_id` may only ever *narrow* a request, never authorize it. (a) Whenever `face_ids` or `photo_ids` are present, every selected object is authorized individually against the photo-context gate of FR-030-43 — the album gate is not an alternative to those checks. (b) When `album_id` is supplied together with explicit IDs, the album gate of FR-030-44 must additionally pass **and** every selected photo must be linked to that album via the `photo_album` pivot. (c) `FaceDetectionService::dispatchPhotos()` combines the explicit-ID predicate and the album predicate with `AND` (intersection), so the album can never widen an explicit selection. | Owner batch-assigns/scans objects inside an album they own with matching `album_id` → allowed. Objects selected without `album_id` → allowed only when every object passes its own photo gate. | `album_id` must resolve to an existing album; explicit IDs must resolve to at least one existing object, otherwise the request is denied. | 403 when any selected object fails its photo gate, when the album gate fails, or when any selected photo is not linked to the supplied album — the mutation and the scan dispatch are both skipped in full (no partial application). | — | GHSA-x6f7-qp5q-w37f |
...
Read via `config('services.face_recognition.base_url')` and `config('services.face_recognition.api_key')`. Never visible in the admin UI or included in config API responses.

| PHP key | `.env` variable | Default | Description |
|---------|----------------|---------|-------------|
| `services.face_recognition.base_url` | `AI_VISION_FACE_URL` | `""` | Base URL of the Python face-recognition service (e.g. `http://ai-vision:8000`). Must not have a trailing slash. |
...
| S-030-66 | Non-admin attacker passes an unrelated album they own as `album_id` to `POST /Face/batch` while naming another user's `face_ids` → 403; the foreign face's `person_id` is unchanged (GHSA-x6f7-qp5q-w37f) |
| S-030-67 | Same attacker passes that unrelated owned album to `POST /FaceDetection/scan` with another user's `photo_ids` → 403; the foreign photo's `face_scan_status` is unchanged and no scan job is queued (GHSA-x6f7-qp5q-w37f) |
| S-030-68 | Mixed-owner `face_ids` (one own, one foreign) with the attacker's own `album_id` → 403; neither face is modified |
| S-030-69 | `album_id` naming an album the caller owns but which does not contain the selected photo → 403; the same selection with the matching `album_id` → 200/204 |
...
Communication uses REST API with webhook callbacks. Photo files are accessed via **shared Docker volume** (Q-030-07 resolved). Authentication via a single shared symmetric API key stored in `.env` as `AI_VISION_FACE_API_KEY` and read via `config('services.face_recognition.api_key')` — **never** from the `configs` table. Header: `X-API-Key: <key>`. *(Q-030-15 resolved: single key, both directions; Q-030-19 resolved: ai_vision_* naming)*
