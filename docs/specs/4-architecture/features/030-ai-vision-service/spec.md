...
| FR-030-16 | System shall support dismissing a face via: (a) a "Dismiss" button in the FaceAssignmentModal; (b) a CTRL+click shortcut on face overlays — when the CTRL key is held, overlay rectangles switch to red dashed borders, and clicking a rectangle directly dismisses the face. *(Q-030-54)* | (a) FaceAssignmentModal includes a "Dismiss" button that calls `PATCH /Face/{id}` to set `is_dismissed = true`. (b) FaceOverlay.vue monitors `keydown`/`keyup` for CTRL key; when CTRL is held, all face rectangles render with red dashed CSS borders; clicking a rectangle in this state calls `PATCH /Face/{id}` directly without opening the modal. After dismiss, the overlay is removed from view. | Modal dismiss: face must exist, user must have dismiss permission per `ai_vision_face_permission_mode`. CTRL+click: same authorization as modal dismiss. | 403 if unauthorized; 404 if face not found. | `face.dismissed` | Owner directive, Q-030-54 |
...
| FR-030-21 | When the photo details panel (sidebar) is open and the photo has detected faces, display circular face crop thumbnails with person name underneath. Click opens FaceAssignmentModal; CTRL+click dismisses the face. Map the `P` key to toggle face overlay visibility. *(Q-030-60, Q-030-61)* | New "People in this photo" section in PhotoDetails.vue. Horizontal row of circular face crops (48px diameter) with name label. Click → FaceAssignmentModal. CTRL+click → dismiss (same API as FR-030-16). `P` key toggles overlay visibility (stored in component state; default from `ai_vision_face_overlay_default_visibility` config). | Photo must have faces; face overlay feature must be enabled (`ai_vision_face_overlay_enabled = 1`). | — | — | Owner directive, Q-030-60, Q-030-61 |
...
| FR-030-24 | Maintenance page shall include a block for resetting photos with `face_scan_status = 'failed'` so they can be re-scanned. The block only appears when failed photos exist. *(Q-030-55)* | `GET /Maintenance::resetFailedFaceScans` (check): returns count of photos with `face_scan_status = 'failed'`. `POST /Maintenance::resetFailedFaceScans` (do): sets `face_scan_status = NULL` on those photos. Block hidden when count is 0. | Admin-only. | — | `face.failed_scans_reset` (with `reset_count`) | Owner directive, Q-030-55 |
...
*Note: When CTRL is held, all face overlay rectangles switch to red dashed borders. Clicking directly dismisses the face without opening the modal.*
...
*Note: Circular face crop thumbnails (48px) with person name underneath. Click opens FaceAssignmentModal. CTRL+click dismisses. "???" for unassigned faces.*
...
|                                                                    |
|  ┌─────────────────────┐  ┌─────────────────────┐                |
|  │ Destroy Dismissed   │  │ Reset Failed Scans  │                |
|  │ Faces               │  │                     │                |
|  │ 23 dismissed faces  │  │ 5 failed scans      │                |
|  │ [Destroy All]       │  │ [Reset All]         │                |
|  └─────────────────────┘  └─────────────────────┘                |
|                                                                    |
|  ┌─────────────────────┐  ┌─────────────────────┐                |
|  │ Reset Stuck Scans   │  │ Bulk Scan Faces     │                |
|  │                     │  │                     │                |
|  │ 2 stuck pending     │  │ 150 unscanned       │                |
|  │ [Reset All]         │  │ [Scan All]          │                |
|  └─────────────────────┘  └─────────────────────┘                |
|                                                                    |
+------------------------------------------------------------------+
...
*Note: Each face maintenance block is conditional — hidden when its count is 0.*
...
| API-030-22 | GET /api/v2/Maintenance::resetFailedFaceScans | Admin: check — returns count of photos with `face_scan_status = 'failed'`. Hidden when count is 0. *(Q-030-55, FR-030-24)* | Admin-only |
| API-030-22b | POST /api/v2/Maintenance::resetFailedFaceScans | Admin: do — sets `face_scan_status = NULL` on all photos with `face_scan_status = 'failed'`, making them eligible for re-scanning. Returns `{reset_count: N}`. | Admin-only |
...
| UI-030-08 | CTRL+click dismiss mode | Hold CTRL → face overlays turn red/dashed → click to dismiss directly *(FR-030-16)* |
| UI-030-09 | Person miniature in dropdown | Face assignment dropdown shows circular miniature + name + face count per person *(FR-030-20)* |
| UI-030-10 | Face circles in detail panel | Photo detail sidebar → "People in this photo" section with circular face crops and names *(FR-030-21)* |
| UI-030-11 | Face overlay visibility toggle | Press `P` → face overlays toggle between visible and hidden *(NFR-030-11)* |
...
| UI-030-15 | Maintenance reset failed scans | Maintenance page → "Reset Failed Scans" card (visible only when count > 0) *(FR-030-24)* |
...
routes:
  ...
  - id: API-030-22
    method: GET
    path: /api/v2/Maintenance::resetFailedFaceScans
    notes: admin-only; check — count of failed face scans (Q-030-55)
  - id: API-030-22b
    method: POST
    path: /api/v2/Maintenance::resetFailedFaceScans
    notes: admin-only; do — reset failed scans to null (Q-030-55)
  - id: API-030-23
    method: POST
