...
- `is_validated` boolean on `Photo` model, set during photo creation pipeline.
...
- **In scope:**
  ...
  - Migration: `is_validated` boolean on `photos` table (default `true`, indexed).
  ...
  - Photo creation pipeline: determine `is_validated` from user trust level or guest config.
...
- **`Photo` model** (`app/Models/Photo.php`) — new `is_validated` boolean property and cast.
- **`ImportParam` DTO** (`app/DTO/ImportParam.php`) — may need new `is_validated` parameter, or determination can happen in a pipe.
...
- **`PhotoResource`** (`app/Http/Resources/Models/PhotoResource.php`) — includes `is_validated`.
...
- **Assumptions:**
  ...
  - The `PhotoQueryPolicy` visibility sub-query can be extended with an additional AND condition on `is_validated`.
...
- _Steps:_
  ...
  3. Create migration `add_is_validated_to_photos`: add `is_validated` boolean to `photos` table, default `true`, indexed, after `is_highlighted`.
  ...
  6. Update `Photo` model: add `is_validated` to `$casts` array as `boolean`. Add `@property` PHPDoc.
...
- _Goal:_ Set `is_validated` on new photos based on the uploader's trust level or guest config.
...
- _Steps:_
  1. Create `app/Actions/Photo/Pipes/Shared/SetUploadValidated.php` — a new pipe in the shared photo creation pipeline. Logic:
     ...
     - If owner exists and `owner->may_administrate === true`: set `is_validated = true` (admin short-circuit, Q-033-03 → A).
     ...
     - If trust level is `CHECK`: set `$state->photo->is_validated = false`.
     - For `TRUSTED` or `MONITOR`: set `$state->photo->is_validated = true` (Q-033-01 → A: `monitor` behaves as `trusted`).
  ...
  4. Write unit test: mock admin user with `check` trust level → photo gets `is_validated = true` (admin override).
  5. Write unit test: mock non-admin user with `check` trust level → photo gets `is_validated = false`.
  6. Write unit test: mock non-admin user with `trusted` trust level → photo gets `is_validated = true`.
  7. Write unit test: mock non-admin user with `monitor` trust level → photo gets `is_validated = true`.
  8. Write unit test: guest upload with `guest_upload_trust_level = check` → photo gets `is_validated = false`.
- _Commands:_ `php artisan test --filter=SetUploadValidated`, `make phpstan`
- _Exit:_ Photos created with correct `is_validated` based on trust level.
...
- _Steps:_
  1. Update `PhotoQueryPolicy::applyVisibilityFilter()`: after the existing visibility sub-query, add an additional condition — for non-admin, non-owner contexts, also require `photos.is_validated = true` OR `photos.owner_id = $user_id`.
  2. Update `PhotoQueryPolicy::applySearchabilityFilter()`: apply the same `is_validated` condition.
  ...
  5. Write feature test: admin approves photo (sets `is_validated = true`) → photo becomes visible to all.
...
- _Steps:_
  ...
  4. Create `ModerationController`:
     - `list(ListModerationRequest)`: query `Photo::where('is_validated', false)->orderBy('created_at', 'desc')`, paginate, eager-load owner and albums, transform to `ModerationResource`.
     - `approve(ApproveModerationRequest)`: update `Photo::whereIn('id', $photo_ids)->update(['is_validated' => true])` in chunks of 100.
...
- _Steps:_
  ...
  7. Update `PhotoResource` in `lychee.d.ts` with `is_validated` field.
- _Commands:_ `npm run check`, manual visual testing.
...
- _Steps:_
  1. Write integration test: full upload-to-moderation-to-public flow.
     ...
     - Upload photo as that user → photo has `is_validated = false`.
...
- _Steps:_
  ...
  6. Write feature test (S-033-24): dispatch `ProcessImageJob` for a guest upload to an album configured with `grants_upload = true`; verify `is_validated` reflects `guest_upload_trust_level` config, not the album owner's trust level.
- _Commands:_ `php artisan test --filter=ProcessImageJob`, `php artisan test --filter=SetUploadValidated`, `make phpstan`
- _Exit:_ Guest uploads via queue correctly set `is_validated` based on `guest_upload_trust_level` config.
...
| S-033-22 | I7 / T-033-27, I2 | PhotoResource includes is_validated |
...
- [ ] `is_validated` column on `photos` table, indexed, default `true`.
- [ ] Two config entries created and accessible via `ConfigManager`.
- [ ] Photo creation pipeline sets `is_validated` based on user trust level or guest config.
