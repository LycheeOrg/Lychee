...
| `ModerationController` | Internal | Must be updated for approval-time album marking and blocked-findings filtering. |
...
- _Steps:_
  ...
  4. Create `NsfwScanStatus` enum (`app/Enum/NsfwScanStatus.php`).
  ...
  8. Create `NsfwVisibility` enum (`app/Enum/NsfwVisibility.php`) — `VISIBLE`, `BLOCKED`, `REVIEW`.
  9. Create migration: add `nsfw_scan_status`, `nsfw_visibility`, `upload_trust_level` to `photos` table.
  10. Create migration: `nsfw_detections` table.
  11. Create config migration: 7 new config keys.
  12. Update TypeScript type `UserUploadTrustLevel` to include `trust_but_verify`.
- _Commands:_ `make phpstan`
- _Exit:_ Enums compile, migration runs on test DB, `SetUploadValidated` handles new tier, PHPStan 0.
...
- _Steps:_
  ...
  3. Create `ApplyNsfwAlbumSensitivityJob` (`app/Jobs/ApplyNsfwAlbumSensitivityJob.php`) — loads the photo's parent album, checks `is_recursive_nsfw`, sets `album.is_nsfw = true` if appropriate. Handles no-album fallback via `nsfw_sensitive_no_album_action`. Dispatched from two places: (a) `NsfwActionService` at callback time for auto-approved tiers, (b) `ModerationController::approve()` for admin-approved `check` photos.
...
- _Steps:_
  ...
  5. Update `ModerationController::approve()` — after setting `is_validated = true` and `nsfw_visibility = visible`, check if photo has sensitive detections and `nsfw_sensitive_album_action = mark_album`. If so, dispatch `ApplyNsfwAlbumSensitivityJob`.
...
- _Steps:_
  ...
  4. Update Moderation page: add `nsfw_visibility` column/badge, hide `blocked` entries by default, add toggle (UI-045-03).
  5. Update `ModerationResource` to include `nsfw_visibility`.
  6. Update `ModerationController::list()` to exclude `nsfw_visibility = blocked` by default; accept `show_blocked` query param.
...
- _Exit:_ Settings section renders with matrix, moderation page filters blocked by default.
...
| S-045-10 | I5 / T-045-18 | Callback: trust_but_verify + block → moderate |
| S-045-11 | I5 / T-045-18 | Callback: trust_but_verify + review → approve |
| S-045-12 | I5 / T-045-18 | Callback: trusted + block → approve |
...
| S-045-32 | I6 / T-045-22 | Moderation page: blocked hidden by default |
| S-045-33 | I6 / T-045-22 | Moderation page: toggle shows blocked |
...
- [ ] All enums compile and are used in config/request validation.
...
- [ ] Moderation page hides blocked findings by default.
...
4. **Retry mechanism** — Artisan command to retry all `nsfw_scan_status = failed` photos.
