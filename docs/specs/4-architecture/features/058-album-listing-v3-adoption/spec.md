...
- Invalidate the shared album-list store on login and logout, mirroring the existing `AlbumService.clearCache()` convention already called from those exact two places, so a session that gains or loses album visibility is never served a list cached under the prior identity.
...
- **No visual UI redesign.** The three surfaces look the same to the user; this feature only swaps their underlying data source and thumbnail-loading mechanism.
...
| FR-058-03 | New Pinia store `resources/js/stores/AlbumListState.ts` (`useAlbumListStore`). `fetch()`/`ensureLoaded()` calls `GET /api/v3/Albums` in **base mode only** (no `with_parent_id`/`for_bulk_edit` — those are admin-gated per FR-057-02/03 and this store must also serve non-admin move/merge dialogs), caches the raw `ids`/`titles`/`lft`/`rgt`/`cover_ids` arrays in store state, and is a no-op on a second call unless `invalidate()` was called first or `force: true` is passed. | Repeated calls to `ensureLoaded()` across multiple mounted consumers in the same session issue at most one network request. | N/A. | A failed fetch leaves the store's `error` state set; consumers fall back to their existing empty/"no target" state. | None. | Q-058-04; DO-058-03. |
| FR-058-04 | The store exposes a computed `tree` — the album hierarchy reconstructed purely from `_lft`/`_rgt` via a nested-set stack algorithm (ascending `_lft` order; pop the stack while the current album's `_lft` exceeds the stack top's `_rgt`; the new stack top, if any, is the parent) — and a pure function `getExcludedTargetIds(rootIds: string[]): Set<string>` returning the union, over every id in `rootIds`, of that album's own id plus every album whose `(lft, rgt)` range is strictly contained within it (i.e. every descendant), computed directly from the flat `lft`/`rgt` arrays (no tree walk needed). `rootIds` accepts one id (single-album Move) or several (multi-album Merge) uniformly. | For any `rootIds`, the returned set makes every remaining album a legal move/merge target with no cyclic parent/child relationship possible. | N/A (pure function, no I/O). | Empty `rootIds` → empty exclusion set (no-op), not an error. | None. | Q-058-05; mirrors `ListAlbums::do()`'s single-root `_lft`/`_rgt` exclusion (`app/Actions/Album/ListAlbums.php:44-53`), generalized to N roots. |
| FR-058-05 | New component `resources/js/v8/components/thumbs/Thumb.vue`, props `albumId: string`, `photoId: string \| null`, `type: SizeVariantAssetType` (default `'thumb'`). Fetches `GET /api/v3/Asset/{album_id}/{photo_id}/{size_variant}` as a blob via axios with an `AbortController` signal (mirrors `upload-service.ts`'s existing signal usage), caches the resolved object URL keyed by `` `${albumId}:${photoId}:${type}` `` in a module-level `Map` so the same asset is never re-fetched within the session, and aborts its own in-flight request in `onBeforeUnmount`. Renders the existing "no image" placeholder asset immediately (no request) when `photoId === null`, and falls back to the same placeholder on a 403/404/aborted response. | Thumbnail renders identically to today for albums with a cover; placeholder renders for albums without one; a second `<Thumb>` for the same `(album_id, photo_id, type)` renders instantly from cache. | N/A. | A 403/404 from the Asset endpoint (e.g. race with a concurrent delete) falls back to the placeholder, not a broken image icon. | None. | Q-057-05; Feature 056 (`GET /api/v3/Asset/{album_id}/{photo_id}/{size_variant}`); DO-058-05/06. |
| FR-058-06 | When `modules.is_struct_of_array_enabled === true`, `SearchTargetAlbum.vue` calls the shared store's `ensureLoaded()` (FR-058-03) instead of `AlbumService.getTargetListAlbums()`; filters the store's flat album list by `getExcludedTargetIds(props.albumIds ?? [])` (FR-058-04); builds each option's breadcrumb string client-side from the store's `tree` (FR-058-04); renders each row's thumbnail via `<Thumb :album-id="row.id" :photo-id="row.cover_id" type="thumb">` (FR-058-05). When `false`, behavior is byte-for-byte unchanged from today. | Dropdown lists the same albums as v2 would, correctly excluding the moving/merging album(s)' own subtree(s). | N/A (no new user input). | Empty curated list → picker shows the existing "no target" state (`error_no_target`), unchanged from today. | None. | Move-target picker consumer; Q-058-04/05. |
...
| FR-058-11 | The shared store's `invalidate()` (FR-058-03) is called from `LoginForm.vue`'s `login()` success handler (`resources/js/v8/components/forms/auth/LoginForm.vue:125`, alongside the existing `AlbumService.clearCache()` call) and from `LeftMenu.vue`'s `logout()` handler (`resources/js/v8/menus/LeftMenu.vue:170`, alongside the existing `AlbumService.clearCache()`/`*Store.reset()` calls) — a login or logout changes which albums `AlbumQueryPolicy::applyVisibilityFilter()` returns for the caller, so a list cached under the prior identity must never be served under the new one. | Store re-fetches on the next `ensureLoaded()` call after either transition. | N/A. | N/A. | None. | User instruction ("clear the AlbumListState [on login] because we will gain access to more"); existing `AlbumService.clearCache()` login/logout convention. |
...
| NFR-058-03 | Client-side tree-building/exclusion for the move-picker (FR-058-04) produces results identical to today's server-side `ListAlbums::do()` for the same fixture data in the single-root case, and — since no server precedent exists for the multi-root (merge) case — is reasoned from first principles and manually verified against a fixture with a selected parent/child pair and a selected pair of unrelated albums. | Regression safety for a security-relevant behavior (can't move or merge an album into its own descendant, or into a co-selected album's subtree). | Manual side-by-side comparison, flag on vs. off, same fixture album set; dedicated multi-root fixture for the merge case. | `Actions\Album\ListAlbums::do()` (`app/Actions/Album/ListAlbums.php`); `NodeTrait::appendNode()`'s `\LogicException` guard (`vendor/lychee-org/nestedset/src/NodeTrait.php:1199-1200`) as the documented server-side backstop (see plan.md Risks). | User instruction (no behavior regressions); Q-058-05. |
...
| NFR-058-09 | The shared store's cached list is invalidated (re-fetch on next `ensureLoaded()`) after any mutation that changes the album tree from within a consumer this feature touches — move, merge — mirroring the existing `AlbumService.clearCache()`/`clearAlbums()` convention already called by `AlbumMove.vue`/`MoveDialog.vue`/`AlbumMergeDialog.vue` today. | Prevents the shared list from going stale mid-session after the user performs the very action it was used to drive. | Manual verification: move an album, reopen the Move dialog, confirm the moved album's new position is reflected. | `resources/js/services/album-service.ts` (`clearCache`/`clearAlbums`, existing convention). | User instruction ("saves the global list ... when fetched", implies staying correct across mutations). |
| NFR-058-10 | The shared store never serves a list fetched under a different auth identity than the current one — a cached pre-login (guest or less-privileged) list must not under-serve a freshly-logged-in user's actual visibility, and a cached pre-logout list must not linger and over-serve (or leak titles from) the prior identity after logout. | `AlbumQueryPolicy::applyVisibilityFilter()`'s result set is identity-dependent; the shared store's whole purpose (session-wide caching) is unsafe unless it tracks identity transitions. | Manual: as a guest (or lower-privileged user), open a dialog that populates the store, then log in as a user with visibility into more albums — reopening the picker shows the newly-visible albums (S-058-14); separately, log out after the store is populated and confirm the next dialog open re-fetches rather than reusing the prior identity's cached list (S-058-15). | FR-058-11. | User instruction. |

## UI / Interaction Mock-ups

No visual change to the three surfaces' layout — the Move/Merge/Copy dialogs, Fix Tree, and Bulk Album Edit look the same with the flag on or off. The one visible micro-change is the move-picker's thumbnail: previously a bare `<img>`, now rendered by `<Thumb>`, which is visually identical once loaded (same placeholder-on-error/no-cover behavior) but may show a brief blank/placeholder state while its request is in flight, matching how any other lazily-loaded thumbnail in this app already behaves. See [docs/specs/4-architecture/spec-guidelines/ui-ascii-mockups.md](../../spec-guidelines/ui-ascii-mockups.md); no new mock-up is meaningful here.
...
- **CLI:** N/A.
- **UI (JS/Selenium):** No automated frontend suite exists in this repo (confirmed, Feature 049). Manual/browser-based verification for S-058-01..15, per Feature 054's precedent (ad hoc Playwright/Chromium session if available, or direct browser click-through plus DevTools Network-panel inspection for S-058-10/12/13, and a real login/logout cycle for S-058-14/15), covering both flag states.
