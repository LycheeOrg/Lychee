# Feature Plan 042 – Webshop Order Item Display
...
An administrator or customer opening an order detail page can immediately identify each purchased item by its album context and a thumbnail — without having to remember file names or cross-reference galleries. Success is measured by:
...
- PHPStan 0 errors, php-cs-fixer clean, all tests pass, `npm run check` exits 0.
...
- TypeScript type generation pipeline (`php artisan typescript:transform` or equivalent npm script).
- `lang/` i18n files.
...
- **Risks / Mitigations:**
  - **TypeScript transformer command:** The exact artisan command for type generation may differ. Verify with `php artisan list | grep typescript` before I3. If unavailable, the TypeScript interface must be updated manually and noted in the tasks.
  - **Eager-load depth:** Loading `items.photo.size_variants` inside `OrderResource::fromModel()` for every order detail call adds SQL joins. Mitigated by constraining size variants to THUMB-category types only (NFR-042-02) and confirming with query-count assertion (S-042-07).
...
- [ ] All six FRs are unambiguous and traceable to tasks.
- [ ] All seven scenarios map to at least one increment/task.
- [ ] No open questions logged in `open-questions.md` for Feature 042.
- [ ] Estimated total effort ≤ 180 min (fits within session).
- [ ] TypeScript transformer command verified (`php artisan typescript:transform` or manual fallback noted in I4).
...
- [ ] All seven backend scenarios covered by passing tests.
