...
| FR-056-09 *(2026-09-22 amendment, security regression; ADR-0010)* | FR-056-08's cache exception stays exactly as specified — `isComputedAlbumThumb()` authorizes on the presence of the `album_user_thumbs` row alone and performs **no** per-request permission re-check on the photo. This endpoint is hit once per rendered thumbnail, so an extra visibility query on that branch was explicitly rejected (ADR-0010). The invariant that makes this safe is moved to the write side: **every path that revokes a viewer's access must purge the cached covers that access produced**, via the new `App\Actions\Sharing\PurgeAlbumUserThumbs` action (`forBaseAlbums()` / `forUsers()`). A row is keyed by the viewer who materialised it (`Auth::id()`, see `CachesAlbumUserThumb`), never by the permission their access came from — an authenticated viewer reaching an album through its **public** permission stores the row under their own `user_id` — so purges must never be narrowed by the revoked permission's user/group. Call sites: `SharingController::delete()` (`forBaseAlbums`, previously narrowed by user/group, which is the regression this amendment fixes); `Propagate::overwrite()` (`forBaseAlbums` over the descendants whose own permissions it wipes); `PurgeAlbumUserThumbsOnMembershipChange` listening on `UserGroupMembershipChanged` (`forUsers`); `UserGroupsController::delete()` (`forUsers` over the members read before the group is dropped). Already covered without a new call: photo deletion (FK `photo_id` cascades), user deletion (`User::delete()`), album deletion (`Actions\Album\Delete`), photo membership changes (`RecomputeAlbumUserThumbsOnPhotoChange`, whose job recomputes each cached viewer through a permission-filtered query). | Revoked access → no cached cover survives → the endpoint's generic membership check denies → 403. Unaffected viewers keep serving from cache; a purged viewer lazily re-seeds on their next read. | N/A — no new request input. | A revocation path added in future without a purge call re-opens this hole; the purge call is the control, and this row is its register. | None. | Security retest 2026-09-22 (public-share revocation for an authenticated viewer, follow-up to commit 8e84d7667); ADR-0010. |
...
| S-056-18 | Owner revokes an album's **public** permission while an authenticated viewer holds an `album_user_thumbs` cover for one of its photos → that viewer's row is purged (`SharingTest`), so the endpoint denies on the next request. |
| S-056-19 | A viewer is removed from a group (or the group is deleted) that granted their only access to the source album → their cached covers are purged (`UserGroupMembershipTest`/`UserGroupTest`), no `access_permissions` row having changed. |
