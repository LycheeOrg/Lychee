...
|----|---------|-----------|
| CLI-004-01 | `php artisan lychee:recompute-album-sizes` | Populates `album_size_statistics` for all existing albums. Idempotent, shows progress bar. Operator must run manually after migration during maintenance window. |
...
cli_commands:
  - id: CLI-004-01
    command: php artisan lychee:recompute-album-sizes
...
1. Each `RecomputeAlbumSizeJob` instance gets unique ID via `'job_' . base64_encode(random_bytes(17))` in constructor
...
1. **CLI command** `php artisan lychee:recompute-album-sizes` for server operators with shell access
