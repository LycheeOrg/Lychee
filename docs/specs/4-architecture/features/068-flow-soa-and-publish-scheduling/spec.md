...
| Field | Value |
|-------|-------|
| Status | Implemented (28 of 33 tasks green; one manual browser-only check, T-068-11b, outstanding — no dev environment available this session) |
...
| FR-068-13 | `UpdateAlbumRequest` (`PATCH /Album`) gains an optional `published_at` field (new `HasPublishedAt` contract + trait, `RequestAttribute::PUBLISHED_AT_ATTRIBUTE`, plus a `publishedAtProvided(): bool` accessor mirroring `tagsProvided()`), validated `sometimes|nullable|date` (ISO-8601 string, e.g. `2026-09-17T10:00:00+02:00`, mirroring how `PhotoEdit.vue`/`PhotoService` already send `taken_at`) — `sometimes`, not `present`: confirmed by implementation that a `present` rule 422s every *existing* caller of this endpoint (11 regression-test failures), since `published_at` is a brand-new key no current caller sends yet, exactly the same reason `slug`/`tags` are `sometimes` rather than `present` on this same endpoint. Key absent → existing value untouched (`processValidatedValues()` checks `array_key_exists()`, not just `??`); key present with `null` → clears both `published_at`/`published_at_orig_tz`. | Owner sets/clears an album's publish date via the API; the cast splits the submitted instant into `(published_at, published_at_orig_tz)` on write. | Non-parseable date string → 422. | N/A. | None. | Mirrors `taken_at`'s existing single-album edit precedent (`PhotoEdit.vue`/`PhotoService::update()`) and `slug`/`tags`'s existing `sometimes` precedent on this same endpoint. |
...
| DO-068-03 | `App\Contracts\Http\Requests\HasPublishedAt` contract (`publishedAt(): ?Carbon`, `publishedAtProvided(): bool`) + `HasPublishedAtTrait`, `RequestAttribute::PUBLISHED_AT_ATTRIBUTE`. | Backend |
...
**Q-068-08 — `UpdateAlbumRequest`'s `published_at` rule: `present` or `sometimes`? (implementation
correction)**

- **Context:** An earlier full-spec-review pass (this same day) "corrected" FR-068-13 from `sometimes`
  to `present`, reasoning in the abstract that `present` matched this endpoint's majority convention.
  That reasoning was never actually tested against the real regression suite.
- **What happened:** Implementing FR-068-13 with `present` and running the existing
  `AlbumUpdateTest`/`AlbumUpdateFocusTest`/`UpdateAlbumRequestTest` suites unmodified surfaced 11
  failures — every existing caller of `PATCH /Album` (including today's v8 frontend, before
  T-068-24 lands) omits `published_at` entirely, since it's a brand-new key nothing has ever been
  asked to send. `present` would make the API immediately incompatible with every not-yet-updated
  caller the moment the backend deploys — precisely the same problem `slug`/`tags` already solve on
  this same endpoint via `sometimes`, for the same underlying reason (a field some/no callers know
  about yet).
- **Fix:** Reverted to `sometimes|nullable|date`, restoring "key absent → existing value untouched"
  semantics, plus a new `publishedAtProvided(): bool` accessor (mirrors `tagsProvided()`) so
  `AlbumController::updateAlbum()` can distinguish "not sent" from "sent as null."
- **Lesson:** A design correction made during a document review, without running it against the real
  test suite, is a hypothesis, not a verified fact — this one didn't survive contact with
  implementation. `[[feedback_verify_before_declaring_fixed]]` applies here too, not just to bug
  fixes.
- **Resolution date:** 2026-09-17 (found and fixed during T-068-20/21's own implementation).
- **Spec impact:** FR-068-13 corrected back to `sometimes`; DO-068-03 gains `publishedAtProvided()`.
