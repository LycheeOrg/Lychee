...
- `resources/js/v8/composables/useAdminTiles.ts` / `resources/js/router/paths.ts` / `resources/js/v8/router/routes.ts` — where `LandingConfig.vue` gets registered: the route name/path pair goes in `paths.ts` (shared manifest both v7 and v8 routers attach components to), the component import + mapping goes in `routes.ts` (v8-specific, alongside `NsfwConfig`/`Webhooks`), and the admin tile goes in `useAdminTiles.ts` (`group: "core"`) — mirroring exactly how `nsfw-config` is registered across all three (confirmed by reading `routes.ts`; see spec Q-054-15).
...
3. **I3 – `LandingLink` admin CRUD (REST)**
   ...
   - _Preconditions:_ I2.
   - _Steps:_ `StoreLandingLinkRequest`/`UpdateLandingLinkRequest` (validation per FR-054-11, incl. default-icon fallback note for the frontend, `icon` ≤255 chars). `LandingLinkResource` (public-safe projection, reused for both admin and public embed). `App\Http\Controllers\Admin\LandingLinkController` — index/store/show/update/patch/destroy mirror `WebhookController`; `reorder()` implements FR-054-12's bespoke full-list-resync contract (`{ ids: string[] }`, reject on set mismatch, transactional) since no existing endpoint in this codebase does bulk reordering (Q-054-17). Routes in `routes/api_v2.php` under the existing admin group.
...
11. **I11 – Admin UI: `LandingConfig.vue` dedicated page (Settings + Links + Featured tabs)**
    ...
    - _Steps:_
      ...
      3. Add an "SE" badge (visible but **not disabled** — the option stays selectable, see Q-054-13) on the `landing_layout` dropdown's `portfolio`/`minimal`/`studio` options and the `landing_animation_preset` dropdown's premium-preset options (read from existing `is_se_enabled`/`is_se_preview_enabled` init data) — this is bespoke to `LandingConfig.vue`'s custom dropdown rendering, since whole-field `require_se` (as NSFW sets client-side on its own keys) doesn't fit a field where only some enum *values* are gated. The tab always shows/edits the **stored** config value, never the post-fallback effective value (UI-054-08).
      ...
      7. Register `landing-config` in `router/paths.ts` (name/path) **and** `resources/js/v8/router/routes.ts` (component import + mapping — both files are required, see Q-054-15), plus an admin tile in `useAdminTiles.ts` (`group: "core"`, visible whenever `can_edit`), mirroring `nsfw-config`'s registration exactly.
    - _Commands:_ `npm run check`, manual browser walk-through.
