...
| FR-069-23 | Every cached search entry is evicted by `ManagedCacheSearchListingInvalidator` on any mutation that can change what a search matches or who may see it. Beyond the photo-side events already wired (`PhotoSaved`, `PhotoMoved`, `PhotoDeleted`, `PhotoTagsChanged`, `PhotoRatingChanged`, `PhotoHighlightToggled`), this covers the album-data events `AlbumSaved`, `AlbumDeleted`, `AlbumTagsChanged`, `AlbumChildrenChanged`, `AlbumComputedDataUpdated`, `AlbumListingCacheFlushRequested`, and the access-permission events `AccessPermissionChanged`, `UserGroupMembershipChanged`. | An album renamed, retagged, moved, deleted, or re-shared is reflected on the next search. | — | — | Existing managed-cache event logging. | FR-069-15, Feature 053 invalidation convention |
| FR-069-24 | Because a cache entry stores an already-browsability-filtered result for one user identity, a cache hit never re-runs `applyBrowsabilityFilter()`. Revoking a grant must therefore evict, not merely re-key: `AccessPermissionChanged` and `UserGroupMembershipChanged` are security-critical eviction triggers, not freshness ones. | A user whose access was revoked no longer sees the album in search, on the request immediately following the revocation. | — | — | — | FR-069-13, FR-069-23 |
| FR-069-25 | The `album_ids[i]` accessibility guarantee of FR-069-04 holds for the origin-scoped branch too: being inside the authorized subtree is not sufficient, because `appendSearchabilityConditions()` admits a viewer-owned photo regardless of its albums' reachability. The same accessibility predicate the unscoped branch applies is applied under an origin, with the same admin short-circuit. | A viewer-owned photo that also sits in an inaccessible in-subtree album reports the accessible album, not the inaccessible one. | — | No accessible in-subtree album ⇒ the `unsorted` fallback of Q-069-11, unchanged. | — | FR-069-04, Q-069-11 |
| FR-069-26 | The v3 search path owns the state its own rendering reads: the photo tier's `ratios` are kept beside `photoTilesV3` and fed to the analytic layout, and the album hits in `albumTilesV3` are the pool for album selection, the context-menu enabled gate, and the no-results check. | Justified/masonry search tiles use real aspect ratios; an album-only result is selectable, actionable and never reported as "nothing found". | — | Off the v3 path every one of these falls back to the browsing stores, unchanged. | — | FR-069-17, FR-069-20 |
| FR-069-27 | On the search route the scoped album's own refresh completes before the search request starts, because `AlbumState.load()` repopulates the shared photo store as a side effect. | A refresh on an album-scoped search leaves the search results on screen. | — | — | — | FR-069-20 |
...
| S-069-33 | An album's title, description, tags, parentage, computed data, or deletion changes ⇒ the next identical search re-queries rather than replaying the stale cached body (FR-069-23). |
| S-069-34 | A share on an album is revoked (`AccessPermissionChanged`), or a user leaves a granted group (`UserGroupMembershipChanged`) ⇒ that user's previously cached search entry is gone, so the album cannot be replayed to them (FR-069-24). |
| S-069-35 | Origin-scoped search, viewer-owned photo that also sits in an inaccessible album inside the same subtree ⇒ `album_ids[i]` names the accessible album (FR-069-25). |
