...
> **Note:** Q-031-01 through Q-031-07 are resolved. Q-031-08 (size_variants query-string encoding) is open but only blocks the `send_size_variants + payload_format=query_string` path — all other tasks can proceed.

## Checklist

### I1 – Database Migration, Enums & Eloquent Model

- [ ] T-031-01 – Create `PhotoWebhookEvent`, `WebhookMethod`, and `WebhookPayloadFormat` enums (FR-031-01, DO-031-03).
...
- [ ] T-031-02 – Create new domain events: `PhotoAdded`, `PhotoMoved`, `PhotoWillBeDeleted` (FR-031-06, FR-031-07, FR-031-08, DO-031-03; Q-031-05 → C, Q-031-06 → D).
  _Intent:_
...
- [ ] T-031-03 – Create `create_webhooks_table` migration (FR-031-01, NFR-031-02, DO-031-01; Q-031-03 → A hard delete).
...
- [ ] T-031-04 – Create `Webhook` Eloquent model with casts and fillable (FR-031-01, NFR-031-03, DO-031-01).
...
- [ ] T-031-05 – Create `WebhookFactory` and `WebhookResource` (DO-031-01, FX-031-01; Q-031-07 → A secret excluded).
...
- [ ] T-031-07 – Create `StoreWebhookRequest`, `UpdateWebhookRequest`, `PatchWebhookRequest` (FR-031-01, FR-031-03, S-031-02–S-031-04, S-031-21; Q-031-01 → A).
...
- [ ] T-031-08 – Create `WebhookController` with all six CRUD methods (FR-031-01 through FR-031-06, API-031-01 through API-031-06; Q-031-03 → A hard delete).
...
- [ ] T-031-09 – Register webhook routes in admin API routes file (API-031-01 through API-031-06).
...
- [ ] T-031-11 – Create `WebhookPayload` DTO and `WebhookPayloadBuilder` service (DO-031-02, FR-031-09, FR-031-11).
  _Intent:_ `app/DTO/WebhookPayload.php` — plain DTO with optional fields: `?photo_id`, `?album_id`, `?title`, `?size_variants` (array). `app/Services/WebhookPayloadBuilder.php` — accepts `Webhook` model + photo snapshot data (array), returns `WebhookPayload`. Applies `send_*` flags, filters size_variant_types, omits absent variants silently. When `payload_format = query_string`, `size_variants` is excluded pending Q-031-08 resolution.
  _Verification commands:_
  - `php artisan test --filter=WebhookPayloadBuilderTest`
  - `make phpstan`

- [ ] T-031-12 – Create `WebhookDispatchJob` queued job (DO-031-04, NFR-031-01, NFR-031-04, NFR-031-05, NFR-031-08, TE-031-01, TE-031-02; Q-031-02, Q-031-04 → A).
  ...
  - `payload_format = json`: `Http::withHeaders(...)->timeout($timeout)->send($method, $url, ['json' => $payloadArray])`
  - `payload_format = query_string`: `Http::withHeaders(...)->timeout($timeout)->send($method, $url.'?'.http_build_query($payloadArray))`
  Sets `User-Agent: Lychee/<version>`, `X-Lychee-Event: <event>` on every request. Sets `$secretHeader: $secret` when secret is configured. On non-2xx or exception: `Log::error('webhook.dispatch.failure', [...])` with `webhook_id`, `event`, `method`, `error` (secret redacted). On success: `Log::debug('webhook.dispatch.success', [...])`.
  _Verification commands:_
  - `php artisan test --filter=WebhookDispatchTest`
  - `make phpstan`

- [ ] T-031-13 – Create `WebhookListener` and register in `EventServiceProvider` (FR-031-06 through FR-031-08, FR-031-13; Q-031-05 → C, Q-031-06 → D).
...
- [ ] T-031-14 – Fire `PhotoAdded` from `SetParent` pipe for new photo records (FR-031-06; Q-031-05 → C).
...
- [ ] T-031-15 – Fire `PhotoMoved` from `MoveOrDuplicate` for cross-album moves (FR-031-07; Q-031-05 → C).
...
- [ ] T-031-16 – Fire `PhotoWillBeDeleted` from `Delete` action before DB deletion (FR-031-08; Q-031-06 → D).
...
- [ ] T-031-17 – Create `php artisan lychee:webhook-test` command (CLI-031-01).
...
- [ ] T-031-23 – Add `webhook_timeout_seconds` to `configs` table seeder/migration (NFR-031-08).
...
- **Q-031-08 (size_variants in query-string):** T-031-11 and T-031-12 should skip `size_variants` when `payload_format = query_string` until this question is resolved. Once resolved, add a follow-up task to implement the encoding and update S-031-15.
