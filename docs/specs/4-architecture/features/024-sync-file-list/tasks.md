...
- [ ] T-024-01 – Verify existing directory-sync tests still pass (regression guard) (S-024-01).  
...
- [ ] T-024-02 – Write failing test: single valid file path imports successfully (FR-024-01, FR-024-02, S-024-02).  
...
- [ ] T-024-03 – Write failing test: unsupported extension is warned and skipped (FR-024-02, S-024-03).  
...
- [ ] T-024-04 – Write failing test: non-existent path produces error and exits 1 (FR-024-01, S-024-04).  
...
- [ ] T-024-05 – Write failing test: mixed directory and file paths processed together (FR-024-04, S-024-05).  
...
- [ ] T-024-06 – Write failing test: multiple file paths (shell expansion simulation) (FR-024-01, S-024-08).  
...
- [ ] T-024-07 – Add `file_list` property to `ImportDTO` (DO-024-01).  
...
- [ ] T-024-08 – Refactor `Sync::validateDirectories()` to `validatePaths()` returning separate file/directory lists (FR-024-01, FR-024-06, S-024-04).  
...
- [ ] T-024-09 – Add `Exec::doFiles()` method (or extend `do()`) for direct file import (FR-024-02, S-024-02, S-024-03, S-024-08).  
...
- [ ] T-024-10 – Wire `Sync::executeImport()` to call `doFiles()` for file-list paths (FR-024-04, S-024-05).  
...
- [ ] T-024-11 – Implement `skip_duplicates` guard in `doFiles()` (FR-024-05, S-024-06).  
...
- [ ] T-024-12 – Print notice when `delete_missing_*` flags are set but all paths are files (FR-024-07, S-024-07).  
...
- [ ] T-024-13 – Run `vendor/bin/php-cs-fixer fix` and resolve any style issues (NFR-024-03).  
  _Intent:_ All PHP files conform to the project style.  
  _Verification commands:_  
  - `vendor/bin/php-cs-fixer fix --dry-run`

- [ ] T-024-14 – Run `make phpstan` — zero new errors (NFR-024-03).  
  _Intent:_ Static analysis passes at the project's configured level.  
  _Verification commands:_  
  - `make phpstan`

- [ ] T-024-15 – Run full test suite and confirm no regressions (NFR-024-01).  
  _Intent:_ `php artisan test` passes entirely.  
  _Verification commands:_  
  - `php artisan test`

- [ ] T-024-16 – Update knowledge map to note file-list support in CLI section.  
...
- [ ] T-024-17 – Update roadmap to set feature 024 status to Complete.  
