...
- [x] T-057-01 – Unit test + implement `AlbumQueryPolicy::joinBaseAlbumBulkEditFields()` (FR-057-03).
...
- [x] T-057-02 – Unit test + implement `CacheKeyProvider::albumListingV3Tag()`/`albumListingV3Key()` (FR-057-05, NFR-057-04).
...
- [x] T-057-03 – Feature tests first: default-mode S-057-01/02/08/13/15/16/17/18 (S-057-01, S-057-02, S-057-08, S-057-13, S-057-15, S-057-16, S-057-17, S-057-18).
...
- [x] T-057-04 – Implement `AlbumListV3Request`, `AlbumListResource`, `AlbumListController::index()`, route registration (FR-057-01, FR-057-07, FR-057-08, FR-057-09, NFR-057-01, NFR-057-03, S-057-01, S-057-02, S-057-08, S-057-13, S-057-15, S-057-16, S-057-17, S-057-18).
...
- [x] T-057-05 – Feature tests first: `with_parent_id` S-057-03/05/12 (S-057-03, S-057-05, S-057-12).
  _Intent:_ Non-admin 403, admin correct `parent_ids` incl. `null` for roots, full index-alignment. Written to fail.
  _Verification commands:_
  - `php artisan test --filter=AlbumListV3Test`
  _Notes:_ Plan I3.

- [x] T-057-06 – Implement `with_parent_id` flag end-to-end (FR-057-02, S-057-03, S-057-05, S-057-12).
...
- [x] T-057-07 – Feature tests first: `for_bulk_edit` S-057-04/06/07 (S-057-04, S-057-06, S-057-07).
  _Intent:_ Non-admin 403, admin full field-parity values, both flags combined. Written to fail.
  _Verification commands:_
  - `php artisan test --filter=AlbumListV3Test`
  _Notes:_ Plan I4.

- [x] T-057-08 – Add `AlbumListBulkEditFieldsResource`; implement `for_bulk_edit` flag end-to-end (FR-057-03, FR-057-04, S-057-04, S-057-06, S-057-07).
...
- [x] T-057-09 – Feature tests first: cache hit/toggle-off S-057-09/11 (S-057-09, S-057-11).
...
- [x] T-057-10 – Wrap controller query in `ManagedCacheService::rememberIf()`; feature test S-057-14 (FR-057-05, NFR-057-04, S-057-09, S-057-11, S-057-14).
...
- [x] T-057-11 – Feature test first + extend `ManagedCacheAlbumListingInvalidator` S-057-10 (FR-057-06, S-057-10).
...
- [x] T-057-12 – Quality gate + docs sync (NFR-057-06).
...
All 12 tasks implemented and verified 2026-08-22. `owner_name` (bulk-edit block) is computed in SQL as `COALESCE(users.display_name, users.username)` rather than invoking `User::getNameAttribute()` (unavailable on raw `toBase()` rows) — this drops that accessor's legacy bcrypt-hash-as-username special case (falls back to `'Admin'`), an edge case not expected to matter for this endpoint's real-world callers.
