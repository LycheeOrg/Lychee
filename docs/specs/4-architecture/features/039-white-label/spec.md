...
| FR-039-02 | `InitConfig` exposes a new boolean property `is_white_label_enabled` populated from `request()->configs()->getValueAsBool('white_label_enabled')`. | Property present in the JSON payload returned by `GET /api/v2/Gallery/Init`. | TypeScript transformer generates the corresponding field in the `InitConfig` TypeScript type. | If the config key is missing, the value defaults to `false` (fail-safe). | No telemetry (config read). | Problem statement |
...
|----|-------------|--------|-------------|--------------|--------|
| NFR-039-01 | The `white_label_enabled` setting must not be visible in the public settings API (`GET /api/v2/Settings`) because `is_secret = 1`; it must only be editable via the admin settings page for SE users. | Privacy — operators must not expose the white-label toggle to end-users. | Feature_v2 test confirms the key is absent from the public settings response. | Existing `is_secret` filtering in the settings query pipeline. | Problem statement |
...
| `resources/views/components/warning-misconfiguration.blade.php` | Use Blade conditional to swap "Lychee" / "lychee.example.com" with generic placeholders |
