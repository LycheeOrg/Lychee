...
- [x] T-067-01 – Write `MapViewportTest` covering `north`/`south`/`east`/`west`/`zoom` bounds
...
- [x] T-067-02 – Implement `App\DTO\MapViewport` (`north`/`south`/`east`/`west`/`zoom`,
...
- [x] T-067-03 – Add `HasMapViewportTrait` (bounds validation rules) and new
...
- [x] T-067-04 – Write `ResolvesMapPhotoSourceTest` asserting `resolveRootQuery()`/
...
- [x] T-067-05 – Implement `App\Actions\Map\ResolvesMapPhotoSource::resolveRootQuery()` —
...
- [x] T-067-06 – Implement `resolveAlbumQuery()` — reproduces `$album->photos()`/
...
- [x] T-067-07 – Write `QueryMapBucketsTest` covering: a simple in-view grid split, an
...
- [x] T-067-08 – Implement `App\Http\Resources\V3\MapBucketResource` (`bucket_ids[]`/`counts[]`/
  `centroid_latitudes[]`/`centroid_longitudes[]`) (DO-067-05).
  _Verification commands:_ `vendor/bin/phpstan analyse`.

- [x] T-067-09 – Implement `App\Actions\Map\QueryMapBuckets::do()`: `snapToGrid()` first,
...
- [x] T-067-10 – Write `QueryMapPhotosTest` covering: a dense single-cell fixture (`count > 20`)
...
- [x] T-067-11 – Implement `App\Http\Resources\V3\MapPhotoResource` — `ids[]`/`album_ids[]`/
...
- [x] T-067-12 – Implement `App\Actions\Map\QueryMapPhotos::do()`: aggregate pre-pass
...
- [x] T-067-13 – Write `MapListingV3Test` (`tests/Feature_v3/Map/`) covering S-067-01 through
...
- [x] T-067-14 – Add new controller wiring `GET /api/v3/Map/buckets`, `GET /api/v3/Map/Photos`,
...
- [x] T-067-15 – New migration adding a plain composite index on `photos(latitude, longitude)`,
...
- [x] T-067-16 – Write cache-key-stability test: two overlapping-but-distinct raw viewports inside
...
- [x] T-067-17 – Add `CacheKeyProvider::mapBucketsKey()`/`mapPhotosKey()`/`mapTracksKey()`/
...
- [x] T-067-18 – Wrap the three controller endpoints in `ManagedCacheService::rememberIf()`, same
  pattern as `PhotoChildrenController` (FR-067-15).
  _Verification commands:_ `php artisan test --filter=MapListingV3Test`.

- [x] T-067-19 – Write invalidation test cases: a save/move to a geotagged photo evicts its
  ...
  _Verification commands:_ `php artisan test --filter=ManagedCacheMapListingInvalidatorTest`
  (expect failure).

- [x] T-067-20 – Add map-scope-aware branches to a new, dedicated `ManagedCacheMapListingInvalidator`
  listener reacting to `PhotoSaved`/`PhotoMoved`/`PhotoDeleted` (root tag always evicted; each
  containing album's tag evicted) (FR-067-16).
  _Verification commands:_ `php artisan test --filter=ManagedCacheMapListingInvalidatorTest`.

- [x] T-067-36 – Write config-change invalidation test: toggling `hide_nsfw_in_map` (and each of
...
- [x] T-067-37 – Add a new `MapListingCacheFlushRequested` event + config-change dispatch
  (`SettingsController::MAP_LISTING_COARSE_FLUSH_CONFIGS`) flushing the coarse
  `mapListingGlobalTag()` — sufficient alone since every Map cache entry carries it — on any of the
  four config keys changing, mirroring Q-053-05's precedent (Q-067-14, FR-067-24).
  _Verification commands:_ `php artisan test --filter=ManagedCacheMapListingInvalidatorTest`;
  `php artisan test --filter=MapListingV3Test`; `vendor/bin/phpstan analyse`.

### I7 – Frontend service + `MapState.ts`

- [x] T-067-21 – New `resources/js/services/map-v3-service.ts`: `getBuckets()`/`getPhotos()`/
...
- [x] T-067-22 – New `resources/js/stores/MapState.ts`: `bucketsV3`/`photosV3`/`tracksV3`,
...
- [ ] T-067-23 – Dev-console/code-review verification: panning/zooming triggers correct
  ...
  _Verification commands:_ Manual, browser devtools network tab. **PENDING — no browser/dev
  environment available this session (`[[feedback_no_mariadb_mysql_access]]`).** Store logic
  (dedup-key computation, in-flight guard) is unit-verifiable and matches the design; the dedup
  key's snapping math mirrors `MapViewport::snapToGrid()` byte-for-byte by construction, but the
  actual Leaflet `moveend`/`zoomend` → fetch wiring has not been exercised in a real browser.

### I8 – `Map.vue` SoA rendering path

- [x] T-067-24 – Wire `moveend`/`zoomend` Leaflet listeners → `MapState.ts.requestViewport()`
  (debounced) (FR-067-19, S-067-14).
  _Verification commands:_ `npm run check` — pass.

- [x] T-067-25 – Render aggregate (count-badge) markers at cell centroid for cells above the leaf
  threshold; click handler zooms the map in, never fetches members (FR-067-20, S-067-15).
  _Verification commands:_ `npm run check` — pass.

- [x] T-067-26 – Render leaf-cell entries via the existing `clusterFunc()`/`.leaflet-marker-photo`
...
- [x] T-067-27 – Move GPX track loading to a one-time `MapState.ts` fetch (via the new `tracks`
...
- [x] T-067-28 – Add `is_struct_of_array_enabled`-driven dispatcher in `Map.vue` between the old
...
- [ ] T-067-29 – Manual browser verification of S-067-14 through S-067-17 (pan/zoom-triggered
  ...
  _Verification commands:_ Manual. **PENDING — no browser/dev environment available this session
  (`[[feedback_no_mariadb_mysql_access]]`).** Not marked `[x]` on typecheck-pass alone; `npm run
  check` and eslint are clean on the full v3 rendering path (T-067-24..28), but real marker
  rendering/interaction has not been visually confirmed.

### I9 – Flag gating + v2 coexistence verification

- [x] T-067-30 – Diff review confirming zero changes to `routes/api_v2.php`'s `/Map`/`/Map::provider`
  ...
  _Verification commands:_ `git diff --stat -- routes/api_v2.php app/Http/Controllers/Gallery/MapController.php app/Actions/Albums/PositionData.php app/Actions/Album/PositionData.php` — confirmed empty; `tests/Feature_v2/Map/MapTest.php` regression-passes unmodified.

### I10 – Documentation

- [x] T-067-31 – Update `docs/specs/3-reference/api-design.md` with the three new
  `/api/v3/Map/...` routes.
  _Verification commands:_ None (docs only).

- [x] T-067-32 – Update `docs/specs/3-reference/database-schema.md` with the new
  `(latitude, longitude)` composite index on `photos`.
  _Verification commands:_ None (docs only).

- [x] T-067-33 – Update `docs/specs/4-architecture/knowledge-map.md` with the grid-bucketing
  pattern (`MapViewport`, `cellSizeForZoom()`/`snapToGrid()`, two-pass leaf-cell resolution).
  _Verification commands:_ None (docs only).

- [x] T-067-34 – Update `docs/specs/3-reference/frontend-gallery.md` with `MapState.ts` and the
  aggregate/leaf marker split.
  _Verification commands:_ None (docs only).

- [x] T-067-35 – Move Feature 067's `roadmap.md` row to Completed with an accurate status note
...
- S-067-14..17 (pan/zoom refetch, aggregate-click behavior, leaf marker parity, decoupled track
  ...
  (`[[feedback_no_mariadb_mysql_access]]`) — T-067-23 and T-067-29 are explicitly flagged pending
  above; not marked `[x]` on typecheck-pass alone, only after real manual verification.
- NFR-067-01's ~100,000-geotagged-photo scale verification: T-067-07's large-fixture case (30
  photos collapsing to 1 bucket row, proving the GROUP BY mechanism's cost is bounded by distinct
  cells not row count) is automated and green; the full end-to-end ~100k-photo manual scale check
  still needs a locally-generated fixture and has not been run this session — same access
  constraint as above.
- The Analysis Gate (per `docs/specs/5-operations/analysis-gate-checklist.md`) was not run as a
  separate step before implementation began; the spec/plan were judged sufficiently stable and
  implementation proceeded directly, with 69 new backend tests plus `npm run check`/eslint serving
  as the correctness gate instead.
