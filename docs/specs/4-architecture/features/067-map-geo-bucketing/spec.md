...
| FR-067-06 | Grid cell size is a pure function of `zoom` only: `cellSizeForZoom(int $zoom): float { return 360.0 / (2 ** ($zoom + 2)); }` — a quarter-tile-width in decimal degrees at Web-Mercator zoom `$zoom` (halves exactly once per zoom level, ties cell boundaries to a deeper level of the same grid Leaflet's own tiles already use; `+2` amended per Q-067-13). No admin-configurable grid formula (Q-067-02, Q-067-13). | Deeper zoom → smaller cells → finer clustering, with no config to manage; e.g. zoom 10 → ~0.088° cells (~9.8 km at the equator), zoom 18 → ~0.0003° (~37.5 m). | N/A. | N/A. | None. | This feature; Q-067-13. |
...
| FR-067-21 | Below `MAX_VIEWPORT_PHOTOS`, every photo in the viewport is handed to `clusterFunc()`/`Cluster.add()` (Q-067-16, amended - no more per-cell grid split) - the exact same `leaflet.markercluster` client-side clustering, marker/popup template, and `.leaflet-marker-photo` CSS the v2 (non-SoA) path already uses. | Cluster sizes and thumbnails match the v2 path's graduated, pixel-radius clustering exactly, instead of a fixed-grid approximation of it. | N/A. | N/A. | None. | This feature; Q-067-16; verified against `Map.vue`'s existing `open()`/template code. |
