...
| FR-067-06 | Grid cell size is a pure function of `zoom` only: `cellSizeForZoom(int $zoom): float { return 360.0 / (2 ** $zoom); }` — one tile-width in decimal degrees at Web-Mercator zoom `$zoom` (halves exactly once per zoom level, ties cell boundaries to the same grid Leaflet's own tiles already use). No admin-configurable grid formula (Q-067-02, Q-067-13). | Deeper zoom → smaller cells → finer clustering, with no config to manage; e.g. zoom 10 → ~0.35° cells (~35 km at the equator), zoom 18 → ~0.0014° (~150 m). | N/A. | N/A. | None. | This feature; Q-067-13. |
...
| NFR-067-05 | The Photos (leaf) tier's row-fetch is a plain `toBase()` pass throughout — no `size_variants` join (Q-067-12), and no Eloquent hydration anywhere, including the root-scope `album_ids[i]` tie-break, which resolves via `AlbumQueryPolicy::appendAccessibilityConditions()` joined against `base_albums`/`computed_access_permissions` directly, collapsed with `GROUP BY`/`MIN()` (Q-067-15) rather than any per-candidate model check. Cost must stay bounded by `(leaf cells actually present in the viewport) × LEAF_THRESHOLD`, structurally (via the `HAVING COUNT(*) <= 20` pre-pass), not merely by convention. | Same class of structural-bound guarantee `QueryPhotoDetails` relies on for its own bounded tier, applied here to a cheaper `toBase()` row-fetch instead of Eloquent hydration. | Test asserting a synthetic 10,000-photo single-cell fixture is excluded from the leaf tier's response entirely (count > threshold). | FR-067-10. | This feature; Q-067-12. |
...
| DO-067-04 | `App\Actions\Map\QueryMapPhotos` — two-pass leaf-cell resolution, `toBase()`-only throughout (no Eloquent hydration, Q-067-12); `LEAF_THRESHOLD = 20`. | Backend |
