...
- **Frontend adoption feature** (mirrors Feature 058): wire sticky date headers + virtual-scroll windowing to the buckets/children endpoints, background-fetch the rights endpoint right after the children endpoint renders, implement the client-side owner/grant-combination logic this spec deliberately leaves server-side-unimplemented, and stop calling the v2 paginated listing once all three are adopted — the actual retirement step for `AlbumChildrenController`/`GetAlbumChildrenRequest`/`AlbumRepository::getChildrenPaginated()`. **Drafted as Feature 062** (`docs/specs/4-architecture/features/062-album-timeline-buckets-adoption/`); drafting it surfaced that the children endpoint had shipped with zero row ordering at all — fixed at the source here instead (I12, FR-061-26), rather than compensating for it client-side in 062.
