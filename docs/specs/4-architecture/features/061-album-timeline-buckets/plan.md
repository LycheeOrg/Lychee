...
- **Frontend adoption feature** (mirrors Feature 058): wire sticky date headers + virtual-scroll windowing to the buckets/children endpoints, background-fetch the rights endpoint right after the children endpoint renders, implement the client-side owner/grant-combination logic this spec deliberately leaves server-side-unimplemented, and stop calling the v2 paginated listing once all three are adopted — the actual retirement step for `AlbumChildrenController`/`GetAlbumChildrenRequest`/`AlbumRepository::getChildrenPaginated()`.
