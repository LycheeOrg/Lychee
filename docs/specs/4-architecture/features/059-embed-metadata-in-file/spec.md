...
| FR-059-03 | Editing title and/or description (`PATCH /Photo` → `PhotoController::update()`) dispatches `EmbedMetadataJob` for the photo, when FR-059-01/02 hold. | After `$photo->save()` succeeds, `EmbedMetadataJob::dispatch($photo)` is queued. Any user who passes `PhotoPolicy::CAN_EDIT` (owner, admin, or an album editor via `EditPhotoRequest::authorize()`) can trigger this — same permission the endpoint already requires today. | Existing `EditPhotoRequest` validation is unchanged. | Under the default `sync` queue driver, `dispatch()` runs the job inline, so this failure path is really NFR-059-07's guarantee: `EmbedMetadataJob::handle()` never throws, so nothing here can break the HTTP response (`PhotoResource`). On a real async queue driver, `dispatch()` itself failing to enqueue (e.g. a broker outage) is a pre-existing, feature-agnostic risk already inherent to every other queued job in this codebase — not something this feature adds new handling for. | — | User requirement |
...
| FR-059-08 | Metadata is written to standard, cross-application-compatible EXIF/IPTC/XMP tag triads per field, so the embedded values are readable by common external tools (Adobe products, Windows Explorer, other DAM software), not just Lychee. | `App\Metadata\Writer` invokes `exiftool` **once per variant**, with all four fields' tag triads in a single invocation: **Title** → `XMP-dc:Title`, `IPTC:ObjectName`, `EXIF:XPTitle`. **Description** → `EXIF:ImageDescription`, `IPTC:Caption-Abstract`, `XMP-dc:Description`. **Tags** → `IPTC:Keywords` (list, cleared then re-appended), `XMP-dc:Subject` (bag, cleared then re-appended), `EXIF:XPKeywords` (`;`-joined). **Rating** (owner's, 1–5) → `XMP-xmp:Rating`, `EXIF:Rating`, `EXIF:RatingPercent` (`rating * 20`). | Every field is fully replaced (clear-then-set for list tags) with the current DB value on every run — never a partial/incremental diff. | Failure is only observable **per variant, not per field**: `exiftool`'s exit code covers the whole invocation, so `Writer::embed()` cannot detect (and this feature does not attempt to detect) a scenario where `exiftool` silently accepts most tags but declines one unsupported tag for the file's format while still exiting `0` — that would surface as a "successful" embed with one field's value simply absent from the file. Deliberately accepted, not built around: the feature's actual reliability boundary is FR-059-01/FR-059-02's gating (the config is off by default, and the job never even attempts a write unless `hasExiftool()` is true) — once those gates pass and `exiftool` is invoked, its overall exit code is trusted as-is; no per-tag stdout parsing or extra validation layer is added on top of it. A full non-zero-exit failure (a whole variant, not one field) **is** caught (FR-059-09/FR-059-13). | — | Cross-tool compatibility; mirrors the well-established multi-standard convention other DAM tools use. |
...
| S-059-19 | An unexpected exception occurs somewhere in `EmbedMetadataJob::handle()` outside the per-variant `Writer::embed()` calls (e.g. while refreshing the photo, or while persisting the post-write checksum/filesize) → still caught, logged as `ERROR`, `JobHistory::FAILURE` recorded, nothing propagates to the caller (NFR-059-07) — critical under the default `sync` queue driver, where a propagated exception would otherwise break the triggering photo-edit HTTP response. |
...
- **Application (Feature, `tests/Feature_v2/Photo/`):**
  ...
  - New `tests/Feature_v2/Photo/EmbedMetadataJobTest.php`: end-to-end job execution against a local fixture image, using `Process::fake()` to assert the exact command array per scenario (S-059-02 through S-059-13), including the RAW-present/absent, non-local-disk-skip, and exiftool-unavailable branches. Also covers NFR-059-07/S-059-19: force an unexpected exception at each stage of `handle()` (not just inside a `Writer::embed()` call) and assert it never escapes.
...
- `INFO`: successful embed (which variant(s) were written; whether `Photo::checksum`/`original_checksum` were also refreshed, per the independent `embed_metadata_update_checksum_enabled` setting — FR-059-17 — since that's no longer implied by "successful embed" alone).
- `WARNING`: a single variant's write skipped/failed while at least one other targeted variant succeeded (FR-059-09, either direction); non-local-disk variant skipped (FR-059-11).
- `ERROR`: total job failure — every targeted variant failed, `exiftool` unavailable, or an unexpected internal error caught by NFR-059-07's top-level guarantee.
