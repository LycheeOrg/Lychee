...
- [ ] T-043-01 – Create `print_sizes` migration (FR-043-01).  
...
- [ ] T-043-02 – Create `pixel_sizes` migration (FR-043-02).  
...
- [ ] T-043-03 – Create `purchasable_print_sizes` migration (FR-043-05).  
...
- [ ] T-043-04 – Create `purchasable_pixel_sizes` migration (FR-043-06).  
...
- [ ] T-043-05 – Extend `order_items` migration (FR-043-11, FR-043-12).  
...
- [ ] T-043-06 – Extend `orders` migration (FR-043-15).  
...
- [ ] T-043-07 – Add `PRINT = 'print'` to `PurchasableLicenseType` (FR-043-10, S-043-28).  
...
- [ ] T-043-08 – Create `PrintSize` model (FR-043-01, DO-043-01).  
...
- [ ] T-043-09 – Create `PixelSize` model (FR-043-02, DO-043-02).  
...
- [ ] T-043-10 – Create `PurchasablePrintSize` model and extend `Purchasable` (FR-043-05, DO-043-03).  
...
- [ ] T-043-11 – Create `PurchasablePixelSize` model and extend `Purchasable` (FR-043-06, DO-043-04).  
...
- [ ] T-043-12 – Extend `OrderItem` model (DO-043-05, FR-043-12).  
...
- [ ] T-043-13 – Extend `Order` model + `canProcessPayment()` guard (DO-043-06, FR-043-15, FR-043-19, S-043-23, S-043-24).  
...
- [ ] T-043-14 – Create FormRequests for PrintSize CRUD (FR-043-01, FR-043-03, FR-043-04).  
...
- [ ] T-043-15 – Create FormRequests for PixelSize CRUD (FR-043-02, FR-043-03, FR-043-04).  
...
- [ ] T-043-16 – Create `PrintSizeResource` and `PixelSizeResource` (FR-043-02, FR-043-18).  
...
- [ ] T-043-17 – Create `PrintSizeManagementController` (FR-043-01, FR-043-03, FR-043-04, FR-043-20).  
...
- [ ] T-043-18 – Create `PixelSizeManagementController` (FR-043-02, FR-043-03, FR-043-04, FR-043-20).  
...
- [ ] T-043-19 – Register admin size routes (API-043-02..09, FR-043-20).  
...
- [ ] T-043-20 – Write feature tests for PrintSize management (S-043-01, S-043-02, S-043-04, S-043-05, S-043-06, S-043-25, S-043-26, S-043-27).  
  _Intent:_ Cover CRUD scenarios for print sizes.  
  _Files:_ `tests/Webshop/PrintSizeManagementControllerTest.php`  
  _Verification commands:_  
  - `php artisan test --filter=PrintSizeManagementControllerTest`  

- [ ] T-043-21 – Write feature tests for PixelSize management (S-043-03, S-043-06).  
...
- [ ] T-043-22 – Extend `PurchasableService` with `syncPrintSizes` / `syncPixelSizes` (FR-043-05, FR-043-06).  
...
- [ ] T-043-23 – Extend purchasable request classes for print/pixel sizes (FR-043-05, FR-043-06).  
...
- [ ] T-043-24 – Extend `ShopManagementController` to call sync methods (FR-043-05, FR-043-06).  
...
- [ ] T-043-25 – Extend `EditablePurchasableResource` to include print/pixel sizes.  
...
- [ ] T-043-26 – Write feature tests for purchasable print/pixel pricing (S-043-07, S-043-08).  
...
- [ ] T-043-27 – Create `CatalogueSizesController` and route (FR-043-17, API-043-01, FR-043-20).  
...
- [ ] T-043-28 – Write feature tests for catalogue sizes endpoint (S-043-09, S-043-19, S-043-20, S-043-21).  
...
- [ ] T-043-29 – Create basket FormRequests for print/pixel items (FR-043-07, FR-043-08).  
...
- [ ] T-043-30 – Extend `BasketService` with `addPrintItem` / `addPixelItem` (FR-043-07, FR-043-08, FR-043-11, FR-043-12).  
...
- [ ] T-043-31 – Write feature tests for basket print/pixel items (S-043-10, S-043-11, S-043-12, S-043-19, S-043-20, S-043-21).  
...
- [ ] T-043-32 – Extend `CreateCheckoutSessionRequest` with shipping address (FR-043-14, FR-043-15).  
...
- [ ] T-043-33 – Extend `CheckoutService` to store shipping address (FR-043-15).  
  _Intent:_ Persist shipping address on Order before payment initiation.  
  _Files:_ `app/Actions/Shop/CheckoutService.php`  
  _Verification commands:_  
  - `make phpstan`  

- [ ] T-043-34 – Write feature tests for checkout shipping address (S-043-13, S-043-14, S-043-15, S-043-16, S-043-23, S-043-24).  
...
- [ ] T-043-35 – Extend `OrderResource` and `OrderItemResource` (FR-043-16).  
...
- [ ] T-043-36 – Write feature tests for order shipping address display (S-043-17, S-043-18).  
...
- [ ] T-043-37 – Unit test `Order::canProcessPayment()` (S-043-23, S-043-24).  
...
- [ ] T-043-38 – Unit test `PurchasableLicenseType::PRINT` enum (S-043-28).  
...
- [ ] T-043-39 – Create admin sizes Vue page scaffold (FR-043-18, S-043-25).  
...
- [ ] T-043-42 – Extend basket modal with item type selector (FR-043-07, FR-043-08, FR-043-09).  
...
- [ ] T-043-43 – Add catalogue sizes fetch and print/pixel basket add service calls.  
...
- [ ] T-043-44 – Add shipping address block to `InfoSection.vue` (FR-043-14, S-043-13, S-043-14).  
...
- [ ] T-043-45 – Create `PrintSizePricesInput.vue` component (FR-043-05).  
...
- [ ] T-043-46 – Create `PixelSizePricesInput.vue` component (FR-043-06).  
...
- [ ] T-043-47 – Integrate print/pixel price inputs into purchasable create/edit form.  
...
- [ ] T-043-48 – Add English translation strings for print/pixel sizes UI.  
...
- [ ] T-043-49 – Copy translation string placeholders to all other locales.  
...
- [ ] T-043-50 – Run full backend quality gate (NFR-043-01..06, S-043-22).  
...
- [ ] T-043-51 – Run full frontend quality gate.  
