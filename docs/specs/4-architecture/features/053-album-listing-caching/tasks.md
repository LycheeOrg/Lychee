...
- [ ] T-053-01 – Migration: `managed_cache_enabled`/`managed_cache_ttl`/`managed_cache_albums_enabled` config rows + `SettingsController` `'Mod Cache'` exemption (FR-053-03, FR-053-25).
...
- [ ] T-053-02 – Scaffold 9 new event classes (EV-053-01..08, EV-053-11).
...
- [ ] T-053-03 – Fix `SetProtectionPolicy` `TypeError` bug + dispatch `TagAlbumSaved`/`PersonAlbumSaved` for protection changes (FR-053-11, S-053-14).
...
- [ ] T-053-04 – Dispatch `TagAlbumSaved`/`PersonAlbumSaved` on create/update (FR-053-09, S-053-11, S-053-12).
...
- [ ] T-053-05 – Dispatch `BaseAlbumRemoved` on tag/person album delete (FR-053-10, S-053-13).
...
- [ ] T-053-06 – Dispatch `AlbumSaved` on the 6 previously-silent `AlbumController` endpoints + `AlbumTagsChanged` on tag sync (FR-053-04, FR-053-28, S-053-04, S-053-05, S-053-33, S-053-34).
...
- [ ] T-053-07 – `Move::do()` event wiring (FR-053-05, FR-053-18 partial, S-053-06).
...
- [ ] T-053-08 – `Merge::do()` event wiring (FR-053-06, FR-053-18 partial, S-053-07).
...
- [ ] T-053-09 – `Transfer::do()` event wiring (FR-053-07, FR-053-18 partial, S-053-08).
...
- [ ] T-053-10 – `BulkEditAlbumsAction` event wiring (FR-053-08, S-053-09, S-053-10).
...
- [ ] T-053-11 – `SharingController`/`Propagate` event wiring (FR-053-12, FR-053-13, S-053-15, S-053-16).
...
- [ ] T-053-12 – `UserGroupsManagementController` event wiring (FR-053-14, S-053-17).
...
- [ ] T-053-13 – Async recompute-job completion event + loop-safety proof (FR-053-15, NFR-053-01, S-053-18).
...
- [ ] T-053-14 – Settings config coarse-flush dispatch (FR-053-16, S-053-19).
...
- [ ] T-053-15 – Regression test: `SetSmartProtectionPolicy::do()` dispatches nothing (~~FR-053-17~~ removed, S-053-20).
...
- [ ] T-053-16 – Photo-delete cross-album cover/header cleanup fix (FR-053-19, S-053-22).
...
- [ ] T-053-17 – `FixTree` coarse-flush dispatch (FR-053-20, S-053-23).
...
- [ ] T-053-18 – `ApplyNsfwAlbumSensitivityJob` dispatch (FR-053-21, S-053-24).
  _Intent:_ `AlbumSaved::dispatch($album)` per album after `$album->is_nsfw = true; $album->save();`.
  _Verification commands:_
  - `php artisan test --filter=NsfwSensitivity`
  _Notes:_ Plan I14e.

- [ ] T-053-19 – `ManagedCacheAlbumListingInvalidator` + `ManagedCacheUserListingInvalidator` (FR-053-22, FR-053-23, FR-053-31).
...
- [ ] T-053-20 – Tag CRUD event wiring: `MergeTag`/`DeleteTag` (FR-053-29, FR-053-30, S-053-35).
...
- [ ] T-053-21 – `ManagedCacheService::rememberIf()` + adopt in `AlbumRepository::getChildrenPaginated()` (FR-053-25 method half, FR-053-01, FR-053-24 children half, NFR-053-03).
...
- [ ] T-053-22 – Adopt `ManagedCacheService` in each of `Actions\Albums\Top::get()`'s four constituent queries, independently (FR-053-02, FR-053-24 root half, NFR-053-03/05/08, FR-053-25 queries 2-5).
...
- [ ] T-053-23 – Adopt `ManagedCacheService` in `GetTagWithPhotosAndAlbums::getAccessibleAlbums()` (FR-053-26, FR-053-25 third consumer, NFR-053-07).
...
- [ ] T-053-24 – Full regression pass, Implementation Drift Gate, documentation (NFR-053-02, NFR-053-04 final sweep).
