...
- **Actions** (`app/Actions/`) - Single-responsibility command objects
  ...
  - **Move grant & cross-owner guards** (Feature 072, ADR-0011) - `access_permissions.grants_move` governs Move/Copy/Merge, separately from `grants_edit`, and covers an album's **content**: `AlbumPolicy::CAN_MOVE` (content of the album may leave it; grant on the album) vs `AlbumPolicy::CAN_MOVE_ALBUM` (the album itself; grant on its parent, like `canDelete()`). `PhotoPolicy::CAN_MOVE` reduces `CAN_MOVE` over containing albums. Invariant: owning an album confers full-photo access and download on every photo in it, so any operation that places content under another owner must be guarded — `AuthorizeCanMovePhotosTrait::mayPlacePhotoIn()` (full access + download required when destination owner ≠ photo owner) and `GuardsCrossOwnerAlbumsTrait::mayReparentAcrossOwners()` (`CAN_TRANSFER` required). A new endpoint that links photos or reparents albums must reuse these guards. Destination pickers filter with `AlbumQueryPolicy::appendEditableCondition()` (v2 `getTargetListAlbums`) / `can_edits` (v3 `/Albums`), SQL twins of `AlbumPolicy::canEdit()` pinned by `EditableConditionParityTest`. Delete follows the same content semantics: `AlbumPolicy::canDeleteById()` checks `grants_delete` on each album's parent, `canDeleteContentById()` on the albums containing photos. The protection policy (public/link/password/NSFW/public grants) is owner-only (`AlbumPolicy::CAN_CHANGE_PROTECTION_POLICY`). Request authorization uses the aggregate `…ById` checks (`AlbumPolicy::canMoveAlbumsById`/`canMoveContentById`/`canDeleteById`, `PhotoPolicy::canMoveById`/`canAccessFullAndDownloadById`), constant query count, pinned against the per-model policies by `AggregateAuthorizationTest`; the per-model abilities remain for UI rights.
