...
| ~~Q-071-04~~ | 071 – Size-Variant Format | Low | GD encodes by **source** type, not target extension (pre-existing: a PNG original yields PNG bytes inside a `.jpeg` thumb). Fix fully, or only route `.webp` targets? | Resolved directly (Option A — only route `.webp`; full fix changes alpha handling for existing GD users and is out of scope; N-071-04, FR-071-09) | 2026-09-26 | 2026-09-26 |
...
| ~~Q-059-01~~ | 059 – Embed Metadata in Original/RAW File | High | Trigger scope per field — should title/description/tags sync for any `CAN_EDIT` user (owner or shared-album editor) the same as today's endpoints, or owner-only across the board? | Resolved (title/description/tags: any `CAN_EDIT` user, matching today's endpoint permission; rating: photo owner only — the user's own request phrasing distinguishes "a user" from "the owner") | 2026-08-27 | 2026-08-27 |
| ~~Q-059-02~~ | 059 – Embed Metadata in Original/RAW File | Medium | Execution model — inline synchronous write inside the controller action, or a queued job? | Resolved (queued `EmbedMetadataJob`, mirroring `App\Jobs\WatermarkerJob`'s single-size-variant-mutation pattern) | 2026-08-27 | 2026-08-27 |
| ~~Q-059-03~~ | 059 – Embed Metadata in Original/RAW File | High | Which file(s) to write — Original only, or also the preserved RAW camera file (Feature 020)? | Resolved (both — user explicitly asked mid-request to also consider the RAW file, since it's the one most likely to be exported/archived outside Lychee) | 2026-08-27 | 2026-08-27 |
| ~~Q-059-04~~ | 059 – Embed Metadata in Original/RAW File | Medium | Tag mapping — a single EXIF tag per field, or the cross-application EXIF+IPTC+XMP triad? | Resolved (write the full triad per field for maximum external-tool compatibility) | 2026-08-27 | 2026-08-27 |
| ~~Q-059-05~~ | 059 – Embed Metadata in Original/RAW File | Medium | Checksum consistency — after a successful embed, should `Photo::checksum`/`original_checksum` be refreshed to match the new file bytes, or left pointing at the pre-edit checksum? | Resolved (refresh — keeps Lychee's own duplicate-finder/integrity tooling internally consistent; does not eliminate the external re-scan/duplicate risk the admin warning exists to flag) | 2026-08-27 | 2026-08-27 |
| ~~Q-059-06~~ | 059 – Embed Metadata in Original/RAW File | High | Write mechanism — the existing `lychee-org/php-exif` package is confirmed read-only; native PHP has no EXIF-write support. Shell out to `exiftool`, or attempt an Imagick-based partial write? | Resolved (hard-require `exiftool`, reusing the existing `has_exiftool`/`exiftool_path` detection — the de facto standard for reliable, wide-format EXIF/IPTC/XMP writing) | 2026-08-27 | 2026-08-27 |
| ~~Q-059-07~~ | 059 – Embed Metadata in Original/RAW File | Medium | Non-local storage disks (S3) — build download/mutate/re-upload plumbing, or scope to local disks only for v1? | Resolved (local disks only — `FlysystemFile::toLocalFile()` already draws this same boundary elsewhere in this codebase; non-local variants are skipped with a warning, not attempted) | 2026-08-27 | 2026-08-27 |
| ~~Q-059-08~~ | 059 – Embed Metadata in Original/RAW File | Medium | Checksum-refresh behaviour was originally bundled unconditionally into the main embed toggle (FR-059-15) — should it instead be its own independent setting? | Resolved (split into a new `embed_metadata_update_checksum_enabled` config, default on, FR-059-17 — direct user instruction; `SizeVariant::filesize` refresh stays unconditional, since it's a factual correction rather than a policy trade-off) | 2026-08-27 | 2026-08-27 |
| ~~Q-059-09~~ | 059 – Embed Metadata in Original/RAW File | High | Review-pass finding: under the default `QUEUE_CONNECTION=sync`, `EmbedMetadataJob::dispatch()` runs `handle()` inline — an uncaught exception anywhere in `handle()` (not just inside a per-variant `Writer::embed()` call) would propagate directly into `PhotoController::update()`/`tags()`/`rate()` and break the photo-edit HTTP response, directly violating Goal 6. Was this actually guaranteed anywhere in the first draft? | Resolved (No — it wasn't; added new NFR-059-07 requiring `handle()`'s entire body to be wrapped in a top-level try/catch, mirroring `WatermarkerJob::handle()`'s own never-throws pattern; new S-059-19 scenario, new NFR-059-07 test task T-059-05) | 2026-08-28 | 2026-08-28 |
| ~~Q-059-10~~ | 059 – Embed Metadata in Original/RAW File | Medium | Review-pass finding: FR-059-08 originally claimed a rejected/unsupported tag causes "that single field's write" to fail — but `exiftool`'s exit code covers the whole invocation (one invocation embeds all 4 fields for a variant at once, per FR-059-13), so per-field write failure isn't actually detectable at the `Writer`/exit-code level. Chase real per-tag detection (parse `exiftool` stdout for warnings) now, or accept the gap? | Resolved (accept the gap, closed — not deferred. The feature's actual reliability boundary is FR-059-01/FR-059-02's gating: off by default, and the job never attempts a write unless `hasExiftool()` is true; once a write is attempted, its overall exit code is trusted as-is, no stdout-parsing/per-tag validation layer added. FR-059-08 and plan.md's Risks corrected to describe this as the deliberate design, not an open gap to revisit) | 2026-08-28 | 2026-08-28 |
| ~~Q-059-11~~ | 059 – Embed Metadata in Original/RAW File | Low | Review-pass finding: several stale/inconsistent cross-references — FR-059-07 cited FR-059-11 (non-local-disk skip) instead of FR-059-12 (`ShouldBeUnique`, the thing it actually meant); Documentation Deliverables and the Appendix heading still said "Q-059-01..07" after Q-059-08 was added; plan.md's Exit Criteria still said "16 Branch & Scenario Matrix rows" after the count grew to 18 (now 19); the Telemetry `INFO` log line didn't account for FR-059-17's checksum-refresh becoming conditional; FR-059-05's failure-path column had a confusing, near-nonsensical parenthetical ("the owner rating someone else's photo, N/A..."); DO-059-01/FR-059-06 described the DTO's `rating` field as sometimes literally `0`, when `photo_ratings.rating`'s `CHECK (rating BETWEEN 1 AND 5)` constraint means the job's execution-time DB read can only ever see `1`–`5` or absent, never a stored `0`. | Resolved (all corrected directly — purely textual/arithmetic/cross-reference fixes, no design decision required, same bucket as this repo's own Q-053-13 precedent) | 2026-08-28 | 2026-08-28 |
...
**Status:** Resolved directly, 2026-09-26 — Option A. Encoded in spec N-071-04 and FR-071-09.  
**Feature:** F-071  
**Priority:** Low
