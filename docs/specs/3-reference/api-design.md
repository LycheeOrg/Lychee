...
- **`limit` param (Feature 068, FR-068-03):** optional, `sometimes|integer|min:1`, mutually exclusive with `bucket_ids[]`/`photo_ids[]` — caps the response to the first `limit` rows in the already-applied effective sort order (a plain `->limit()` on the fully-ordered query). Omitted (every pre-Feature-068 caller) preserves the existing whole-scope response byte-for-byte. Used by Flow's per-card photo preview (`limit=12`, a fixed frontend constant — see below) to avoid embedding an album's entire photo collection in a feed card.
...
### API v3: Map Geo-Bucketing (Struct-of-Arrays)

Three `GET` endpoints replacing the v2 Map's unbounded, whole-scope `GET /api/Map` fetch with a viewport-tiered contract, cost bounded by **distinct grid-cell count in the current viewport**, never by total geotagged-photo count (Feature 067). Registered on `App\Http\Controllers\Gallery\MapListingController` (`buckets()`/`photos()`/`tracks()`), gated by `features.struct-of-array` + `AlbumPolicy::CAN_ACCESS_MAP` (same gate `GET /api/Map` already uses). The v2 route, `MapController`, and both `PositionData` action classes remain fully intact and reachable when the flag is off (Feature 065/066's own coexistence precedent).

Every request carries `north`/`south`/`east`/`west` (decimal degrees) + `zoom` (int, `0`–`24`), plus an optional `album_id` (real `Album` scope; omitted = root/cross-library scope) — resolved into one `App\DTO\MapViewport` value object shared by every query class. `include_sub_albums` is **not** a request parameter (unlike the album/photo tiers' own scoping): it stays server-derived from the single, global `map_include_subalbums` config, exactly like `MapController::getData()` already resolves it today.

**Grid bucketing.** Coordinates are 2D and continuous, unlike the 1D date-truncation bucketing the album/photo/Timeline tiers use — a purpose-built grid mechanism, not a reuse of `PhotoBucketComputer`/`QueryPhotoBuckets`. `MapViewport::cellSizeForZoom(int $zoom): float` returns `360.0 / (2 ** $zoom)` — one Web-Mercator tile-width in degrees, so grid boundaries line up with Leaflet's own tile grid (zoom 0 → 360°/one cell; zoom 10 → ~0.35°; zoom 18 → ~0.0014°). `MapViewport::snapToGrid()` snaps the requested bounding box outward to whole grid cells at the resolved cell size **before** it is used for both the SQL `WHERE`/`GROUP BY` and the cache key — a raw, un-snapped viewport would make the cache layer a near-permanent miss under ordinary small-pan mouse drags. The bounding-box `WHERE` handles an antimeridian-crossing viewport (`west > east`, e.g. panning across the Pacific) via `(longitude >= west OR longitude <= east)` instead of `whereBetween()`.

#### `GET /api/v3/Map/buckets`
...
Registered via `MapListingController::photos()` / `GetMapPhotosRequest`, backed by `App\Actions\Map\QueryMapPhotos`. `toBase()`-only throughout, no per-cell grid grouping (Q-067-16, amended): a `SELECT DISTINCT` query over the viewport's bounding box (collapsing the row-per-membership fan-out an `include_sub_albums` subtree join can otherwise produce for one photo) is bounded by `->limit(MAX_VIEWPORT_PHOTOS + 1)` — fixed at `500`, not admin-configurable — and its rows are only returned if the fetched count is at/under that cap (a separate `count()` pre-check would leave a race against concurrent writes between the two queries, and force an unbounded `COUNT(*)` at low zoom); over it, the response is empty and the frontend falls back to `/Map/buckets`' aggregate badges instead. Below the cap, every distinct photo in the viewport is returned and handed to the same `leaflet.markercluster` client-side clustering the v2 (non-SoA) map already uses. A separate join+collapse pass resolves each photo's real, viewer-accessible containing `album_id`: album scope (± sub-albums) picks the in-scope album with the lowest `_lft` per photo (no extra per-descendant access check, mirroring `all_photos()`'s own existing semantics); root scope has no "natural" album, so it joins `photo_album` → `base_albums` → `computed_access_permissions`, applies `AlbumQueryPolicy::appendAccessibilityConditions()`, and picks the lowest accessible `album_id` per photo. No `size_variants` join and no `should_downgrade` computation anywhere in this tier — leaf-tier marker imagery is fetched by the frontend directly from the existing `GET /api/v3/Asset/{album_id}/{photo_id}/{size_variant}` endpoint (Feature 056) via `ThumbAssetService`, keyed on this response's own `ids[]`/`album_ids[]`; that endpoint's own `GetPhotoAssetRequest` never performs a thumb-vs-full-photo access split for the thumbnail-class variants Map ever requests, so there is no duplicated permission logic to reproduce here.

**Response:** `MapPhotoResource` — `ids[]`, `album_ids[]` (`string|null`, `null` for a photo with no resolvable containing album, e.g. root scope's unsorted photos), `titles[]` (guest-blanked identically to `PhotoResource::$title`'s `file_name_hidden` rule), `taken_ats[]` (preformatted via `date_format_sidebar_taken_at`, native PHP `DateTime::createFromFormat()` against the raw column string — never Carbon, since this tier never hydrates a `Photo` model to begin with), `latitudes[]`, `longitudes[]`. Populated **only** for leaf cells — a cell above the threshold contributes zero entries here (it renders as an aggregate marker instead, driven by the `buckets` tier's own `counts[]`).

#### `GET /api/v3/Map/tracks`

Registered via `MapListingController::tracks()` / `GetMapTracksRequest`. `album_id` is **required** here (422 if missing) — there is no root-scope equivalent, matching `Albums\PositionData::do()`'s own always-empty `tracks` collection at root. Viewport/zoom params, if passed, are ignored — the album's track list doesn't change as the map pans/zooms. Returns the album's existing `TrackResource[]` unchanged.

**Caching.** All three endpoints wrap `ManagedCacheService::rememberIf()` (`managed_cache_albums_enabled`, same toggle every other listing tier reuses), keyed by scope (`'root'` or `album_id`) + the **snapped** viewport/zoom + user identity + unlocked-album digest (`CacheKeyProvider::mapBucketsKey()`/`mapPhotosKey()`/`mapTracksKey()`). Every cached entry carries both a per-scope tag (`mapListingTag($scope)`) and one coarse `mapListingGlobalTag()` shared by every Map cache entry regardless of scope. `PhotoSaved`/`PhotoMoved`/`PhotoDeleted` evict root's tag (always, since it's scope-wide) plus each affected album's tag (`App\Listeners\ManagedCacheMapListingInvalidator`); a change to `hide_nsfw_in_map`/`map_include_subalbums`/`map_display`/`map_display_public` dispatches `App\Events\MapListingCacheFlushRequested`, flushing the coarse global tag alone — sufficient to invalidate every scope at once, mirroring `AlbumListingCacheFlushRequested`'s own precedent for the album-listing cache.

**New index.** `photos(latitude, longitude)` had no index at all prior to this feature — a plain composite B-tree index (not a driver-specific spatial index type; sqlite has none built in without SpatiaLite, which cannot be assumed present) so the bounding-box `WHERE`/grid `GROUP BY` above use it instead of a full table scan.

See `docs/specs/4-architecture/features/067-map-geo-bucketing/spec.md` for the full requirement/decision-card detail, including the `album_ids[]` tie-break rule's derivation (Q-067-15).

### API v3: Flow (Struct-of-Arrays)

**`GET /api/v3/Flow`** (Feature 068) — an **unpaginated**, whole-scope Struct-of-Arrays listing of the caller's current Flow scope/strategy, registered on `App\Http\Controllers\Gallery\FlowListController::index()`, gated by a new `App\Http\Requests\Flow\GetFlowListRequest` (mirrors `FlowRequest`'s existing `flow_enabled`/`flow_public` authorization exactly, plus the `features.struct-of-array` flag). Reuses `App\Actions\Albums\Flow::do()`'s existing query/policy/ordering logic unchanged — that method gained an optional `$with_relations = true` parameter (default preserves the v2 caller byte-for-byte); this route calls `do(false)` to skip the nested photo/cover/statistics eager-loads entirely, then adds its own `v3_`-prefixed joins (`AlbumQueryPolicy::joinBaseAlbumOwnerId()`, `users`, `statistics`) via `addSelect()` — never the replacing `select()`, since `Flow::do()` may already have added its own virtual `is_recursive_nsfw` select (only when `hide_nsfw_in_flow` is off) that `select()` would silently drop.

**Response:** `FlowListResource` (Struct-of-Arrays) — `ids`, `titles`, `descriptions` (Markdown-converted, cached per album — see below), `cover_ids`, `owner_names` (`null` for a guest), `is_nsfws` (computed: `hide_nsfw_in_flow===false && is_recursive_nsfw`), `num_photos`, `num_children`, `min_max_texts`, `published_created_ats`, `diff_published_created_ats`, `statistics` (per-entry `null` when the viewer can't read that album's metrics). **No nested photo data at all** — each card's photo preview is fetched separately, lazily, via the existing `ratios` tier's new `limit` param (see above) once that card actually scrolls into view.

No `page`/cursor param, no `flow_max_items` cap applied (that config remains meaningful only for the untouched v2 `GET /api/Flow` path) — each album is its own "bucket equivalent" in the sense that Feature 061/064/066 established for date/title buckets, except here every row is already one whole album, not a reduced aggregate count. This mirrors `GET /api/v3/Albums`'s own unpaginated, `toBase()`-queried precedent (Feature 057/062) more closely than Timeline's aggregated-count `buckets` tier — response size scaling with total published-album count is an accepted tradeoff, the same one this codebase already accepts for root/tag/person/pinned album listings.

**Cover resolution** is simply the explicit `cover_id` column, no fallback chain — discovered mid-implementation that v2's `FlowItemResource` never actually used `AlbumListController::resolveCoverId()`'s three-tier fallback (`cover_id` → `auto_cover_id_max_privilege` → `auto_cover_id_least_privilege`); `Flow`'s own `cover()` relation is `hasOne(Photo::class, 'id', 'cover_id')` and nothing else. Likewise, `CAN_READ_METRICS`'s Gate-based check is replicated as a plain `match` over `metrics_access`/`owner_id`/`may_administrate` (`AlbumPolicy::canReadMetrics()`'s own logic needs no Eloquent model, just `owner_id` and a config value) — no `Gate::check()` call needed. Both findings are what makes this endpoint fully `toBase()`-based, matching `AlbumListResource`'s own no-Eloquent-hydration pattern.

**Date formatting** uses native `date()`/`Safe\strtotime()`, never Carbon, for `min_max_texts`/`published_created_ats` — Carbon is used only for `diffForHumans()` (no native equivalent), mirroring `FlowItemResource`'s own pre-existing v2 usage, not new Carbon usage introduced by this feature.

**Description caching.** Both `FlowItemResource` (v2) and `FlowListController` (v3) wrap their `Markdown::convert()` call in `ManagedCacheService::rememberIf()`, keyed by `CacheKeyProvider::flowDescriptionKey($album_id)` and tagged `flowDescriptionTag($album_id)`. Invalidation piggybacks on the existing `AlbumSaved` event handler (`ManagedCacheAlbumListingInvalidator::handleAlbumSaved()`) — that event already fires from both the single-album edit path (`AlbumController::updateAlbum()`) and the bulk-edit path (`BulkEditAlbumsAction::do()`), so one handler covers both description-save call sites with no per-site wiring.

**Publish-date scheduling** (`flow_strategy=opt-in`'s previously-unsettable `published_at`) reuses the existing `base_albums.published_at` column in place rather than introducing a new one — it is also independently used by `LandingPageResource`'s automatic-featured-items/latest-album-cover ordering (Feature 054), so forking it into a Flow-specific column would have split one "editorial publish instant" concept into two. A new `published_at_orig_tz` (`string(31)`, nullable) companion column lets `published_at` be cast via the existing `DateTimeWithTimezoneCast` (mirrors `Photo::taken_at`'s established pattern) — `BaseAlbumImpl` gained `implements HasUTCBasedTimes` (the `UTCBasedTimes` trait was already in use, satisfying the interface for free). `PATCH /Album` and `PATCH /BulkAlbumEdit` both gained an optional `published_at` field (`sometimes|nullable|date` — confirmed by regression-testing against the real update-endpoint test suites that `present` would 422 every existing caller that doesn't yet send this brand-new key).

See `docs/specs/4-architecture/features/068-flow-soa-and-publish-scheduling/spec.md` for the full requirement/decision-card detail, including why the originally-proposed `flow_datetime` column was rejected (Q-068-01) and why the album-level tier ended up unpaginated (Q-068-04).
