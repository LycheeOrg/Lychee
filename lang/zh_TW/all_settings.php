...
return [
    ...
    */
    ...
    'documentation' => [
        ...
        'zip_bomb_max_total_size' => '已上傳的 zip 封存檔解壓縮後的最大總大小。',
        'zip_bomb_max_file_size' => '已上傳的 zip 封存檔中單一檔案解壓縮後的最大大小。',
        'zip_bomb_max_entries' => '已上傳的 zip 封存檔中允許的最大項目數。',
        'zip_bomb_max_ratio' => '已上傳的 zip 封存檔允許的最大壓縮比(解壓縮/壓縮)。',
        'zip_bomb_delete_rejected_file' => '當上傳的 zip 檔案被判定為 zip 炸彈並遭拒絕時，將其刪除。',
    ...
    'details' => [
        ...
        'upload_chunk_size' => '格式：數字後接 B、KB、MB、GB 或 TB(例如「10MB」)。使用「0 B」表示自動。',
        ...
        'default_user_quota' => '格式：數字後接 B、KB、MB、GB 或 TB(例如「10GB」)。使用「0 B」可停用配額。',
        ...
        'zip_bomb_max_total_size' => '格式：數字後接 B、KB、MB、GB 或 TB(例如「10GB」)。用於防範 zip 炸彈攻擊。若封存檔宣告或實際的解壓縮後大小超過此限制，該封存檔將被拒絕。',
        'zip_bomb_max_file_size' => '格式：數字後接 B、KB、MB、GB 或 TB(例如「5GB」)。用於防範 zip 炸彈攻擊。若任何項目解壓縮後超過此限制，該封存檔將被拒絕。',
        'zip_bomb_max_entries' => '用於防範「大量小型檔案」類型的阻斷服務攻擊。若封存檔包含的項目數超過此值，將被拒絕。',
        'zip_bomb_max_ratio' => '用於防範依賴極端壓縮比的 zip 炸彈攻擊。若封存檔的整體壓縮比超過此值，將被拒絕。',
        'zip_bomb_delete_rejected_file' => '啟用後，一旦偵測到原始 zip 檔案超過設定的 zip 炸彈防護限制，該檔案將立即被刪除。停用此選項可將檔案保留在磁碟上以供手動檢查。',
