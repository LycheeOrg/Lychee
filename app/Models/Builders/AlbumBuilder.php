...
class AlbumBuilder extends NSQueryBuilder
{
	...
	public function addVirtualIsRecursiveNSFW(): AlbumBuilder
	{
		...
		$this->addSelect(['is_recursive_nsfw' => fn ($q) => $q
			...
			// DB::table('albums', 'a')->whereColumn('a.parent_id', '=', 'albums.id')
			->selectRaw('exists (' . $album_query_policy->appendRecursiveSensitiveAlbumsCondition(
				$q // DB::table('albums', 'a')->whereColumn('a.parent_id', '=', 'albums.id')
				, null, null)->toRawSql() . ')'),
		]);
