...
trait HasRandomIDAndLegacyTimeBasedID
{
	...
	private function generateKey(): void
	{
		...
		if (
			...
			// @codeCoverageIgnoreStart
			$legacy_i_d = sprintf('%010d', microtime(true));
		// @codeCoverageIgnoreEnd
		} else {
			...
			// time :-) )
			$legacy_i_d = sprintf('%015.4f', microtime(true));
			$legacy_i_d = str_replace('.', '', $legacy_i_d);
		}
		$this->attributes[$this->getKeyName()] = $id;
		$this->attributes[RandomID::LEGACY_ID_NAME] = intval($legacy_i_d);
	}
}
