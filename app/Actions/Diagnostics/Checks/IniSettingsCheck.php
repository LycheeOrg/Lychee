...
class IniSettingsCheck implements DiagnosticCheckInterface
{
	...
	public function check(array &$errors): void
	{
		...
		if (
			$this->convert_size(\Safe\ini_get('upload_max_filesize')) < $this->convert_size('30M')
		) {
			$errors[]
				= 'Warning: You may experience problems when uploading a photo of large size. Take a look in the FAQ for details.';
		}
		if (
			$this->convert_size(\Safe\ini_get('post_max_size')) < $this->convert_size('100M')
		) {
			$errors[]
				= 'Warning: You may experience problems when uploading a photo of large size. Take a look in the FAQ for details.';
		}
		$max_execution_time = intval(\Safe\ini_get('max_execution_time'));
		...
		if (empty(\Safe\ini_get('allow_url_fopen'))) {
			$errors[]
