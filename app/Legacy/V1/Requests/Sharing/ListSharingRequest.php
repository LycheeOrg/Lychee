...
final class ListSharingRequest extends BaseApiRequest implements HasBaseAlbum
{
	...
	{
		$this->album = array_key_exists(RequestAttribute::ALBUM_ID_ATTRIBUTE, $values) ?
		...
		if (array_key_exists(self::OWNER_ID_ATTRIBUTE, $values)) {
			// @codeCoverageIgnoreStart
			/** @var int $ownerID */
			$owner_i_d = $values[self::OWNER_ID_ATTRIBUTE];
			$this->owner = User::query()->findOrFail($owner_i_d);
			// @codeCoverageIgnoreEnd
		}
