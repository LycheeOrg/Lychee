...
final class ChineseSimplified implements Language
{
	...
	public function get_locale(): array
	{
		$locale = [
			...
			'KEEP_ALBUM' => '保留相册',
			'DELETE_ALBUM_CONFIRMATION' => '是否确认删除相册 \'%0\' 以及相册中包含的所有照片？操作后不可恢复！',

			'DELETE_ALBUMS_QUESTION' => '删除相册和照片',
			'KEEP_ALBUMS' => '保留相册',
			'DELETE_ALBUMS_CONFIRMATION' => '是否确认删除全部 %0 选中的相册和其中的所有照片？操作后不可恢复！',
			...
			'ALBUMS_NEW_TITLE' => '设置标题为 %0 已选择的相册：',
			...
			'ALBUM_MERGE' => '你确定要合并相册 \'%0\' 到相册 \'%1\'?',
			'ALBUMS_MERGE' => '你确定要合并所有已选择的相册到相册 \'%0\'?',
			'MERGE_ALBUM' => '合并相册',
			'DONT_MERGE' => '不要合并',
			'ALBUM_MOVE' => '你确定要移动相册 \'%0\' 到相册 \'%1\'?',
			...
			'PHOTOS_NEW_TAGS' => '设置你的标签于 %0 已选择的照片。已存在的标签会被覆盖。你可以用逗号分隔多个标签：',
			'PHOTO_SET_TAGS' => '设置标签',
			'PHOTO_CAMERA' => '相机信息',
			'PHOTO_CAPTURED' => '拍摄时间',
			'PHOTO_MAKE' => '设备',
			...
			'PHOTO_DELETE' => '删除照片',
			'PHOTO_KEEP' => '保留照片',
			'PHOTO_DELETE_CONFIRMATION' => '是否要删除照片 \'%0\'？此操作不可恢复！',
			'PHOTO_DELETE_ALL' => '是否要删除全部 %0 已选择的照片？此操作不可恢复！',
			'PHOTOS_NEW_TITLE_' => '设置照片标题于 %0 已选择的照片：',
			...
			'SORT_ALBUM_BY' => '相册排序：根据 %0 的 %1 排序。',
			...
			'SORT_PHOTO_BY' => '照片排序：根据 %0 的 %1 排序。',
			...
			'ABOUT_DESCRIPTION' => '<a target=\'_blank\' href=\'%0\'>Lychee</a> 是一个自由的照片管理工具，其其运行于您的服务器或Web空间。仅需几分钟即可安装。Lychee 为您提供了像原生应用那样上传、管理和分享照片所需的一切，您的所有照片都将安全地存储。',
			'FOOTER_COPYRIGHT' => '本网站上的所有图像均受制于版权 %0 &copy; %1',
