...
class AllSettingsTest extends BaseLivewireTest
{
	...
	{
		$comp = Livewire::actingAs($this->admin)->test($this->component);
		/** @var array<int,Configs> $configs */
		$configs = $comp->viewData('form')->configs;
		$idx = collect($configs)->search(fn (Configs $conf) => $conf->key === 'check_for_updates');
		$comp->set('form.values.' . $idx, '2')
			->dispatch('saveAll')
