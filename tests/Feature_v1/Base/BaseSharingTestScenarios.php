...
abstract class BaseSharingTestScenarios extends BaseSharingTest
{
	...
	abstract protected function generateExpectedTreeJson(array $expected_albums = []): array;
	...
	public function testThreePhotosInPublicAlbum(): void
	{
		...
		$array_unexpected = $this->generateUnexpectedRootJson(null, $this->photoID1, null, $this->photoID1);
		if ($array_unexpected !== null) {
			$response_for_root->assertJsonMissing($array_unexpected);
		}
	...
	public function testPublicAlbumAndPasswordProtectedAlbum(): void
	{
		...
		$array_unexpected = $this->generateUnexpectedRootJson(null, null, null, $this->photoID2);
		if ($array_unexpected !== null) {
			$response_for_root->assertJsonMissing($array_unexpected);
		}
		$response_for_root->assertJsonMissing(['id' => $this->photoID1]); // photo 1 is in password protected, still locked album
		...
		$response_for_album2->assertJson([
			'id' => $this->albumID2,
	...
	public function testPublicAlbumAndPasswordProtectedUnlockedAlbum(): void
	{
		...
		$array_unexpected = $this->generateUnexpectedRootJson(null, null, null, $this->photoID1);
		if ($array_unexpected !== null) {
			$response_for_root->assertJsonMissing($array_unexpected);
		}
	...
	public function testPublicAlbumAndHiddenAlbum(): void
	{
		...
		$response_for_root->assertJsonMissing(['id' => $this->albumID2]); // album 2 is hidden
		$response_for_root->assertJsonMissing(['id' => $this->photoID2]); // album 2 is hidden
		...
		$response_for_tree->assertDontSee(['id' => $this->albumID2]);
	}
