...
class UpdateTest extends AbstractTestCase
{
	...
	public function testApplyMigration(): void
	{
		...
		/** @var User $adminUser */
		$admin_user = User::query()->findOrFail(1);
		$login = $admin_user->username;
		$pw = $admin_user->password;
		$admin_user->username = Hash::make('test_login');
		$admin_user->password = Hash::make('test_password');
		$admin_user->save();
		...
		$admin_user = User::query()->findOrFail(1);
		self::assertEquals('test_login', $admin_user->username);
