...
/**
 * FacePermissionMode::RESTRICTED — the most restrictive mode.
 *
 * Expected behaviour per operation:
 *   View People page   → admin only
 *   View face overlays → photo owner or album editor (canEdit)
 *   Create/edit Person → admin only
 *   Assign face        → admin only (photo + album level both return false)
 ...
 *   Batch face ops     → admin only (returns false for all non-admin)
 *   View album people  → album owner only (isAlbumOwner)
 */
class RestrictedModeTest extends BaseApiWithDataTest
{
	...
	public function testViewPeopleHttpGuestUnauthorized(): void
	{
	...
	public function testAssignFaceGatePhotoOwnerDenied(): void
	{
		$this->actingAs($this->userMayUpload1);
		$this->assertFalse(Gate::check(PhotoPolicy::CAN_ASSIGN_FACE_ON_PHOTO, $this->photo1));
	}

	public function testAssignFaceGatePhotoAlbumEditorDenied(): void
	{
		$this->actingAs($this->userMayUpload2);
		$fresh_photo = Photo::find($this->photo1->id);
		$this->assertFalse(Gate::check(PhotoPolicy::CAN_ASSIGN_FACE_ON_PHOTO, $fresh_photo));
	}
	...
	public function testAssignFaceGateAlbumOwnerDenied(): void
	{
		$this->actingAs($this->userMayUpload1);
	...
	public function testAssignFaceHttpOwnerForbidden(): void
	{
	...
	public function testAssignFaceHttpAdminAllowed(): void
	{
	...
	public function testBatchFaceOpsGateAlbumOwnerDenied(): void
	{
		$this->actingAs($this->userMayUpload1);
	...
	public function testBatchFaceOpsHttpOwnerForbidden(): void
	{
	...
	public function testBatchFaceOpsHttpAdminAllowed(): void
	{
		$face2 = Face::factory()->for_photo($this->photo1)->without_crop()->create();
		$response = $this->actingAs($this->admin)->postJson('Face/batch', [
	...
	public function testViewAlbumPeopleHttpGuestUnauthorized(): void
	{
