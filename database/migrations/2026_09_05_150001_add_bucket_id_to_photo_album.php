 ...
 * `photo_album` pivot table (Feature 064, FR-064-01) — deliberately **not**
 * on `photos` itself, unlike Feature 061's `albums.bucket_id`: `photo_album`
 * is a genuine many-to-many pivot (a single `Photo` can be linked into two
 ...
 * photo alone (NFR-064-06). Populated at write time by
 * {@see \App\Services\PhotoBucketComputer} via the three triggers in
 * FR-064-03, never computed live at read time.
 *
 ...
 * `albums.parent_id` at the time of Feature 061's own bucket_id migration,
